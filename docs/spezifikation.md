...
**Offen, in Stufe 18c zu entscheiden:** Die Spielzeit aus PSN (7.7) soll als weiteres Sortierkriterium hinzukommen. Zu klären ist dabei, wohin Titel **ohne** Spielzeit einsortiert werden – PS3 und Vita liefern grundsätzlich keine. Nach der Regel aus Abschnitt 3 ist das ein „unbekannt" und darf nicht als „null Stunden" gelesen werden; die Wunschlisten-Sortierung stellt Unbewertetes aus demselben Grund hinten an.
...
`app_setting` unter `cron_letzter_ausgang`** (Stufe 18b). Das ist kein Gespeichertes, das sich
...
vermerkt („cron: abgebrochen"). `GET /api/sync/status` nennt `letzterAutomatischerLauf`; die
