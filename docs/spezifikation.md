...
- **Offene Scans** stehen in den Einstellungen (Anzahl, zuletzt gescannt) mit „zuordnen" (öffnet `/scannen?ean=…`, Kamera bleibt aus) und „verwerfen" (`DELETE /api/scan/unresolved/:ean`, für Codes, die kein Spiel sind).
...
POST   /api/scan                      Body: { ean } – 400 bei Form oder Prüfziffer; { ean, treffer: mapping|angebot|keiner, release? { releaseId, spielId, titel, plattform, bild, exemplare },
