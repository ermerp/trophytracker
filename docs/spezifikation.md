...
*Version 49d – Zwei nachgerechnete Zahlen zum Cron, ohne Codeänderung (24.09.2026): Die Aufrufbilanz einer Nacht in 10.1 nannte **siebzehn** Aufrufe und zählte Spielzeit und Besitz mit je einem, während 7.7 für dieselben Schritte zwei und fünfzehn nennt – es sind **einunddreißig**. Folge: **Zwanzig Verlaufseinträge fassen eine Kaufliste-Nacht nicht**, die ältesten elf – die Sync-Zeilen – fallen weg; wie das zu lösen ist, **ist in einer späteren Stufe zu entscheiden** (nächste betroffene Nacht: 30.09.2026). Dazu die Streuung der IGDB-Stempel: **368 der 477 Spiele tragen den 22.09.2026**, weil eine Nacht den Rückstand aus 18b in einem Zug abarbeitete – sie werden gemeinsam fällig und brauchen acht statt „ein bis zwei" Aufrufe; zusammen mit einer Kaufliste-Nacht wären es **37 von 36** Aufrufen. Wer dem Cron einen Schritt hinzufügt, rechnet gegen diese Zahl (10.1). Version 49c – Nachbesserung zu Stufe 18d, zwei Zahlen im Cron-Verlauf (Befunde vom 24.09.2026 aus der Analyse der Nachtläufe): Die Sync-Zeile nennt in der Normalisierungsphase die **offenen Rohantworten** statt des dort feststehenden `offset=0` – fünf gleichlautende Zeilen konnten einen gesunden Lauf nicht von einer Seite unterscheiden, die immer wieder scheitert. Die Spielzeit-Zeile nennt zusätzlich `geschrieben`, also den Stand **nach** dem Plattformfilter; `geholt=200 zugeordnet=117` las sich sonst als 83 nicht zugeordnete Spiele, obwohl es die Streaming-Apps waren (10.1). Nur `cronLogzeile`, keine Migration, keine Schemaänderung. Version 49b – Nachträge aus Stufe 19, die der Spezifikation widersprachen (24.09.2026): Die **Pfeilknöpfe** der To-Do-Liste gibt es nicht mehr, die ganze Karte zieht (5.4, 13); die **Notiz** steht nicht mehr auf der Karte, nur noch in Datenmodell, API und Spieldetail (13); die **Plattform** eines Listeneintrags ist ein Kennzeichen, das auf Tippen zum Auswahlfeld wird (13); „Liste importieren" sitzt hinter dem Pluszeichen (13); **`theme_color` und `background_color` sind getrennt** – erstere ist die Grundfarbe der Anwendung (13). Neu unter den Risiken: Androids Navigationsleiste und ihr Trenner sind aus einer Web-App **nicht färbbar** (17) – gemessen am Gerät des Nutzers, damit es keine spätere Stufe erneut versucht. Version 49a – Nachbesserung der Oberfläche noch am Tag des Baus (Rückmeldungen des Nutzers vom 24.09.2026): Die Filter stehen in **zwei Ebenen** – nur gesetzte sichtbar, alles Weitere hinter einem Knopf „Filter" (23 Chips nebeneinander waren „zu lang"). Die Plattform eines Listeneintrags ist eine **Box**, die erst auf Tippen zum Auswahlfeld wird. „Liste importieren" sitzt hinter demselben Pluszeichen wie „Wunsch hinzufügen". Und der Viewport trägt `viewport-fit=cover`: Ohne das blieben die `env(safe-area-inset-*)` null und das Betriebssystem malte einen hellen Streifen unter die Leiste. Version 49 – Stufe 19, die Oberfläche (Entscheidungen des Nutzers vom 22.–24.09.2026 an einem Prototyp): Gestaltungslinie **„Vitrine"** mit Tokens in `frontend/src/tokens.css` – **nur dunkel**, Gold als einziger Akzent, Plattform- und Zustandsfarben als Information, `--aus` als ein Ton für alles Fehlende, Saira für Titel und Zahlen, eigener Zeichensatz (13). Die untere Leiste trägt **vier Symbole ohne Text** statt sieben Einträge bei 0,6 rem: Kaufliste und Lücken werden Reiter der Wunschliste, Scannen eine Kopfzeilenaktion der Sammlung, Einstellungen das Zahnrad; am Desktop stehen die Reiter als Unterpunkte in der Seitenleiste. Die Suche klappt erst auf Tippen auf, Filter sind Chips, Kacheln und Zeilen sind je Liste umschaltbar (im Gerät gespeichert). Der Hinweisblock zieht auf die neue Startseite `/start`; das **Dashboard ist als Stufe 19a herausgelöst**. Der Besitz ist in der Sammlung nur noch Anzeige – die Schnellerfassung entfällt, erfasst wird über Scanner und Spieldetail. `/api/games` nimmt `platform` jetzt **kommagetrennt mehrfach** (12) – die einzige Änderung im Worker, keine Migration. Version 48 – Neues App-Symbol (Wahl des Nutzers vom 24.09.2026, Abschnitt 13): der Pokal steht jetzt in einem zu 87 % geschlossenen Fortschrittsring. Er entstand aus zehn Entwürfen, die im Oberflächen-Prototyp zu Stufe 19 nebeneinanderstanden; die übrigen neun verzichteten teils ganz auf den Pokal (Regal, Stapel, Regalwand). Alle PNG-Fassungen neu erzeugt, maskierbare Schutzzone geprüft. Version 47 – Stufe 18d: der Cron wird les- und aufräumbar (Befunde vom 23.09.2026 aus der Analyse der Nachtläufe). Der Verlauf verdichtet aufeinanderfolgende Leerlauf-Aufrufe zu einer Zeile und fasst zwanzig statt fünf Einträge – fünf zeigten systematisch nur das leere Ende der Nacht (10.1). Ein neunter Cron-Schritt löscht alte `psn_raw_response`; die jüngsten drei Läufe und alles Nichtnormalisierte bleiben (7.1, 10.1) – 2,31 von 3,26 MB der Datenbank waren Rohantworten, die nie jemand löschte. Die Datums-Modifier in `zumAuffrischen` und `haengendeAbbrechen` stehen jetzt als Text im Statement: In Stufe 18b war das angekündigt, aber nur in der Zählabfrage der Statusanzeige umgesetzt. Keine Migration. Version 46 – Stufen 17c bis 18c abgenommen (22.09.2026); die beiden offenen Fragen zur Spielzeit-Sortierung sind damit geschlossen. Version 45 – Stufe 18c gebaut: Spielzeit und digitaler Besitz aus PSN (22.09.2026). Neue Tabelle `psn_played_title` (Migration 0023) nach dem Vorbild von `trophy_progress` – Sonys Rohwerte für sich, `release_id` als korrigierbare Zuordnung; `digital_entitlement` bekommt `herkunft` (`nutzer` / `psn`). Zwei Cron-Schritte: Spielzeit täglich (2 Aufrufe), Kaufliste wöchentlich (15 Aufrufe), beide mit Blätterung und Fortschritt in `app_setting`. Gemessen gegen das echte Konto: von 379 gelieferten Einträgen sind 303 Spiele (76 Streaming-Apps und Unbestimmtes fallen weg), **236 bekommen ein Release**; die Kaufliste ergibt **56 Kauf- und 160 PS+-Einträge**, 514 Einträge betreffen Spiele, die die Sammlung nicht kennt, und bleiben liegen. Spielzeit steht im Spieldetail und ist Sortierkriterium der Sammlung (Unbekanntes ans Ende); von PSN erkannte Berechtigungen tragen **kein Löschkreuz** – der nächste Lauf legte sie ohnehin wieder an. Die Rohantworten werden **nicht** abgelegt (ausdrückliche Ausnahme von „Rohdaten vor Normalisierung", Begründung wie bei IGDB). Version 44 – Entscheidungen zu Stufe 18c und ein Cron-Verlauf (Nutzer, 22.09.2026): Spiele **ohne** Spielzeit (PS3, Vita) rutschen bei der Sortierung nach Spielzeit ans **Ende**, wie Unbewertetes auf der Wunschliste. Beim digitalen Besitz gibt es nur **zwei** Zustände: `kauf` schlägt `plus` – wer ein Spiel gekauft hat, behält es, da ist das Abo gleichgültig; ob ein PS+-Titel dauerhaft eingesammelt oder nur zeitweise im Katalog ist, wird **nicht** unterschieden. Weil der Katalog sich monatlich ändert, ist die PS+-Seite eine **Momentaufnahme**: Ein vollständiger Lauf ersetzt die PSN-erkannten `plus`-Einträge als Ganzes, ein abgebrochener ändert nichts; `kauf` wird nie entfernt (7.7). Der Cron hält jetzt die **letzten fünf Ausgänge** statt nur des letzten – der letzte einer Nacht lautet fast immer „nichts" (Migration 0022, 10.1). Version 43 – Stufe 18 abgeschlossen und die Trophäenstufe verschoben (22.09.2026): Der nächtliche Lauf hat erstmals vollständig gearbeitet – Sync mit 431 Titeln und **368 aufgefrischte Spiele** zwischen 03:00 und 05:55; damit ist der Befund vom Vortag erledigt, und als Ursache bleibt der gebundene Parameter in `datetime('now', ?)`, den Stufe 18b durch Text im Statement ersetzt hat. Die Einzeltrophäen rücken auf Wunsch des Nutzers **hinter** die Oberflächenstufe und heißen jetzt **19b** – ihre Anzeige hinge sonst an einem Spieldetail, das gleich danach umgebaut wird. Version 42 – Zwei neue Stufen geplant, 18c und 19b (Entscheidung des Nutzers vom 21.09.2026, nach einer Messung gegen das echte Konto): Die PSN-Anbindung nutzt bisher nur die Trophäenliste. **18c – Spielzeit und digitaler Besitz** holt `playDuration`/`playCount`/`firstPlayedDateTime` aus `gamelist/v2` (284 von 379 Titeln passen eindeutig auf die Sammlung) und die Kaufliste per GraphQL, deren Feld `subscriptionService` gekauft von PS+ unterscheidet (gemessen 117 zu 613; 247 von 730 passen auf die Sammlung). **19b – Einzeltrophäen** holt Definition und eigenen Stand je Trophäe samt weltweiter Seltenheit (18 355 Trophäen in 431 Listen), gestaffelt über den Cron und danach nur für Listen, deren Änderung der Sync ohnehin erkennt; sie steht seit dem 22.09.2026 **hinter** der Oberflächenstufe, weil ihre Anzeige sonst auf einem provisorischen Spieldetail landete. Beide Stufen **importieren nichts**: Was die Sammlung nicht kennt, bleibt liegen. Drei gemessene Grenzen: keine Spielzeit für PS3 und Vita, keine digitalen PS3-Käufe, und ein fehlender Eintrag heißt nie „nicht besessen" (neuer Abschnitt 7.7). 18c kommt **vor** der Oberflächenstufe, 19b danach; 19 bis 21 behalten ihre Nummern, weil „Stufe 20" an vierzehn Stellen referenziert ist. Version 41 – Stufe 18b, der Cron wird nachprüfbar (Befund vom 21.09.2026): Der nächtliche Sync lief zwei Nächte sauber durch (je 431 Titel, 03:00–03:50 UTC), aber das **IGDB-Auffrischen tat nichts** – 418 von 477 Spielen blieben überfällig. Von außen war nicht zu erkennen, ob der Schritt scheiterte, gar nicht lief oder nichts zu tun fand: Worker-Logs sind nur live einsehbar. Drei Gegenmaßnahmen: (1) Spiele, die IGDB auf Anfrage **nicht zurückgibt**, werden trotzdem gestempelt (`auffrischStempeln`) – sonst wählt der nächste Aufruf dieselben und kommt nie voran, dieselbe Regel wie bei der Disc-Prüfung; (2) die 7-Tage-Frist steht als **Text** im Statement statt als Bind in `datetime('now', ?)`; (3) die IGDB-Schritte laufen in `try/catch`, damit eine Ausnahme nicht den ganzen Aufruf reißt. Vor allem aber hinterlässt jeder Cron-Aufruf seinen Ausgang in `app_setting` (`cron_letzter_ausgang`), sichtbar in `GET /api/sync/status` und in den Einstellungen unter „Automatik" – der Cron ist der einzige Schreiber ohne Zuschauer (10.1). Version 40 – Stufe 17d, offene Scans abgeschafft (Entscheidung des Nutzers vom 21.09.2026): „Später", der Sammelmodus „Nur sammeln", die Ansicht `/scans` und der nächtliche Job sind **weg**. Begründung des Nutzers: Ein Code ohne seine Hülle lässt sich später nicht mehr zuordnen – genau die Erfahrung, die zur Live-Auflösung geführt hat –, und seit der Scanner Titel sofort liefert und „Spiel anlegen" im selben Fenster steht, gibt es nichts mehr zu vertagen. `POST /api/scan` schreibt einen unbekannten Code deshalb nicht mehr weg; wer gerade nicht zuordnen kann, überspringt und scannt die Disc später erneut. **upcitemdb bleibt als Rückfall**, aber live im Worker (`src/ean/upcitemdb.ts`): gefragt wird nur, wenn eBay den Code nicht kennt, genau einmal, und jeder Fehler heißt schlicht „kein Titel" – damit entfallen `scans.yml`, `scans-aufloesen.mjs` und fünf Routen. Die Tabelle `unresolved_scan` bleibt vorerst leer stehen (ein Drop bräuchte zwei Deployments), Abschnitt 9.3 hält fest, warum es die Liste gab und warum nicht mehr. Version 39c – Sichtbarkeit beim Scannen (Rückmeldung des Nutzers vom 21.09.2026): „Ich habe nicht erkannt, dass der Scan schon Teil der Sammlung ist." Ein Treffer aus der Sammlung wird deshalb als **dieselbe Karte** gezeigt wie ein bekannter Code – Cover, Titel als Link, Pille „in deiner Sammlung", je Release „noch kein Exemplar" / „im Regal ×n" und der Knopf „Disc erfassen (PS5)" bzw. „Weiteres Exemplar"; der Angebotstitel rutscht als Nebenzeile darunter. Vorher stand dort nur „EAN … ist noch nicht zugeordnet", eine Aussage über den **Code**, während die Frage des Nutzers dem **Spiel** gilt. Ohne Treffer sagt die Zeile es ausdrücklich: „Noch nicht in deiner Sammlung." `GET /api/scan/:ean/online` liefert dafür Cover und Exemplarzahl je Release mit (`releasesFuerScan`). Version 39b – Nachbesserung Stufe 17c nach dem ersten Regal-Durchgang (Rückmeldung des Nutzers vom 21.09.2026): Von acht erfassten Discs schlug eine falsch vor – `Kingdom Come: Deliverance II` wurde zu `Kingdom Come: Deliverance`, weil die Sammlung „II" schreibt und die Verkäufer „2". Die **römischen Zahlen II–IX** kommen deshalb doch in die Wortzerlegung (`v` und `x` nicht: „Mega Man X" ist kein „Mega Man 10"); sie waren in der ersten Messung nur deshalb durchgefallen, weil damals noch die Plattformziffer aus „Sony PlayStation 3" mitlief. Dazu braucht der Sieger der Mehrheitsregel jetzt **Rückhalt**: Ab drei Angeboten müssen ihn mindestens zwei nennen – sonst setzt sich ein Alleinkandidat aus einem Bündel durch („GTA 4 + 5 Red Dead Redemption"), während das eigentliche Spiel in zwei Fassungen mit Jahreszahl in der Sammlung steht und auf keines seiner eigenen Angebote passt. Gemessen über alle 42 echten Codes: 37 richtig, **kein Fehlgriff** (vorher 37 richtig, 1 falsch). Version 39a – Nachbesserung Stufe 17c (Rückmeldung des Nutzers vom 21.09.2026): Der Scanner bekommt neben „Später" ein **„Verwerfen"**, weil eine Fehllesung sonst nur über „Offene Scans" wieder wegzubekommen war – derselbe Fehlgriff wie am 17.09.2026 (zwei vertauschte Ziffern, Prüfziffer bleibt gültig) trat erneut auf, und wer die Hülle in der Hand hält, erkennt ihn sofort (9.1). Version 39 – Stufe 17c, eBay als EAN-Quelle (Entscheidungen des Nutzers vom 21.09.2026): Ein unbekannter Barcode wird **beim Scannen** aufgelöst statt nachts – die Kette ist jetzt `ean_mapping` → `market_offer` → **eBay live** (`GET /api/scan/:ean/online`, Browse API, Application-Token im Speicher wie bei IGDB) → upcitemdb im Nachtjob; die Live-Abfrage darf in den Worker, weil eBay in rund 0,3 s antwortet und 5 000 Abfragen am Tag erlaubt, während upcitemdb nach je sechs Abfragen 90 s blockt (Netzwartezeit zählt nicht gegen die 10-ms-CPU-Grenze). Der Abgleich bekam zwei gemessene Korrekturen: **Mehrheitsregel** über die bis zu zehn Angebote je Code (ein Bündel „A + B" nennt ein fremdes Spiel und fällt heraus) und **Ballast vor Ziffern** samt Plattformziffer (`Sony PlayStation 3` ließ eine blanke 3 stehen, `LittleBigPlanet2` fand seinen Nachfolger nicht). Gemessen gegen die echten Codes: 30 von 30 bekannten Codes richtig, kein Fehlgriff, 15 von 22 bisher unlösbaren Codes eindeutig (9.2). Version 38 – Stufe 18, Cron Trigger und PWA (Entscheidungen des Nutzers vom 19.09.2026): Ein Cron Trigger `*/5 3-5 * * *` (36 Aufrufe je Nacht, 05–08 Uhr MESZ) tut je Aufruf genau eine schwere Arbeit – erschienene Titel freigeben, hängengebliebene Läufe abbrechen, ein Sync-Schritt, sonst IGDB-Auffrischen (Frist 7 Tage), sonst Disc-Fassungen (neuer Abschnitt 10.1); ein Cron-Versuch je Nacht ohne Wiederholung, bei `abgelaufen` gar keiner; `psn_sync_run.started_by` (Migration 0021), `GET /api/sync/status` nennt `letzterAutomatischerLauf`, Hinweisblock meldet abgelaufenen Zugang und fehlgeschlagenen Nachtlauf, Block „Automatik" in den Einstellungen. PWA mit `vite-plugin-pwa`: Manifest, Service Worker mit Network-First für Navigationen und alle `GET /api/*` (außer Export), Cache-First für Cover und WASM; `index.html` bewusst nicht im Precache und Antworten nur ohne Weiterleitung von der eigenen Origin – sonst hebelte der Cache die Access-Anmeldung aus; Offline-Balken; eigenes Pokal-Symbol. **Gestaltungsregel: keine PlayStation-Logos oder -Schriftzüge, auch nicht angedeutet** (13). Version 37 – Stufe 17b, offene Scans zuordnen (Entscheidungen des Nutzers vom 18.09.2026): Titel zu gescannten Codes holt ein täglicher GitHub-Job bei `upcitemdb` (`GET /api/scan/ungeprueft`, `POST /api/scan/:ean/vorschlag`), weil die freie Quelle nach jeweils sechs Abfragen 90 Sekunden drosselt und 100 am Tag erlaubt; gespeichert wird nur der Titel (Migration 0020), der Abgleich mit der Sammlung entsteht beim Lesen (`src/domain/scan-titel.ts`, Wortmengen ohne Ballast, längster Treffer gewinnt); Ansicht `/scans` in Blöcken wie die Import-Durchsicht, Erfassen über dieselbe Route wie der Scanner, Rückgängig stellt den offenen Scan wieder her; gemessen 22 von 56 Codes eindeutig (neuer Abschnitt 9.3). eBay als zweite Quelle vertagt. Version 36 – Stufe 17, Barcode-Scan (Entscheidungen des Nutzers vom 16.09.2026): Ansicht `/scannen` als siebter Leisteneintrag mit Handy-Kamera und Laptop-Webcam über die Standard-API `BarcodeDetector`, Fallback ist der Polyfill `barcode-detector` (ZXing als WebAssembly, vom eigenen Worker ausgeliefert) statt `html5-qrcode`; Auflösungskette ohne externe Quelle – `ean_mapping`, `market_offer`, Suche in der eigenen Sammlung (`GET /api/games?search=`), Anlegen wie „Spiel anlegen" in der Sammlung; Zuordnen legt Disc **und** Mapping an, erledigt Kauf/Wunsch und protokolliert `exemplar_angelegt` mit `detail = 'scan'` („per Barcode"); unbekannte Codes in `unresolved_scan`, „Offene Scans" in den Einstellungen; Routen `POST /api/scan`, `GET /api/scan/unresolved`, `POST /api/scan/:ean/assign`, `DELETE /api/scan/:ean`, `DELETE /api/scan/unresolved/:ean`; keine Migration. Offen: Messung von eBay-GTIN und upcitemdb gegen die echten Codes des Regals (9.2). Version 35 – Stufe 16, Änderungsprotokoll (Entscheidungen des Nutzers vom 16.09.2026): neue Tabelle `game_event` (Migration 0019) mit Quelle je Zeile (`nutzer` / `sync` / `igdb` / `import`, `feed` und `migration` reserviert), geschrieben ausschließlich in `src/db/` im Batch der jeweiligen Änderung, set-basierte Schreiber per `INSERT … SELECT` mit derselben Bedingung vor dem UPDATE (neuer Abschnitt 8.5); der Sync protokolliert nur Erkanntes (neue Liste, automatische Zuordnung, Vorbelegung, Prüflisten-Einträge einmalig), IGDB nur Entscheidungen und Statuswechsel, Aufbewahrung unbegrenzt und exportiert (15. Fachtabelle), Start bei null ohne Rückrechnung; Block „Verlauf" im Spieldetail (`GET /api/games/:id/events`) und Ansicht `/aenderungen` (`GET /api/events`) mit Quellenfilter und Keyset-Blätterung. Version 34 – Stufe 15, Kaufliste und „Erscheint bald" (Entscheidungen des Nutzers vom 16.09.2026): Kaufliste `/kaufliste` in der Leiste, gespeist aus Lücken und Wünschen über zwei Kandidatenblöcke (`v_kaufkandidaten` mit Cover, ohne Angekündigte, Migration 0018); ein Wunsch kommt als **Kopie** auf die Kaufliste (`origin='wunsch'`, der Wunsch bleibt) statt per Feld-Update; ein erledigter Kauf erledigt den Wunsch am selben Ziel mit; das Erfassen einer Disc oder Berechtigung erledigt offene Kauf- und Wunscheinträge am Release und am Spiel **automatisch** (zweite Ausnahme neben 5.5, Bestandsangleich in 0018); „Erscheint bald" `/erscheint-bald` aus `v_erscheint_bald` (datumsbewusst), `angekuendigt → erschienen` bei „Metadaten auffrischen" bis zum Cron in Stufe 18; neue Reihenfolge ab Stufe 16 (Änderungsprotokoll, Barcode, Cron/PWA, Oberfläche, AWIN, Store – Entscheidung des Nutzers vom 16.09.2026). Version 33 – Stufe 14, Lücken (Entscheidungen des Nutzers vom 16.09.2026): `physical_release_status` kommt aus IGDBs Händlereinträgen (`external_games`, nur `unbekannt` → `ja`, gemessen 199 von 481 Releases), von Hand über `PATCH /api/releases/:id`; Lückenansicht `/luecken` mit „physisch nicht gewünscht" (5.3) und dem Block „Disc-Fassung unbekannt" (Block B); `v_luecken` mit `verworfen`, `disc_fassung`, `game_id`, `cover_url`, `v_kaufkandidaten` ohne verworfene (Migration 0017); `psn_product_id` pflegbar. Version 32 – Stufe 13, Änderungserkennung (Entscheidungen des Nutzers vom 16.09.2026): Der Sync vergleicht den Trophäenstand mit dem Stempel der letzten Durchsicht und legt `neue_trophaeen` und `dlc_erweitert` in der Prüfliste vor; `dlc_erweitert` ohne Statusfilter; neue Spalte `trophy_progress.reviewed_progress_pct` (Migration 0016) für den Vorher-Nachher-Vergleich in Prozent (4.1, 8.1). Version 31 – Nur PlayStation (Entscheidung des Nutzers vom 16.09.2026): IGDB-Einträge ohne nachgewiesene PS3/PS4/PS5/Vita-Plattform sind nirgends ein Treffer – auch nicht ohne Plattformangabe; jede Suche trägt den Plattformfilter (7.6). Suchfeld in Wunschliste, To-Do und Backlog (`suche=`, 5.2). Version 30 – Titel aus IGDB (Entscheidung des Nutzers vom 16.09.2026): Ein Spiel ohne Trophäenliste übernimmt beim Verknüpfen den IGDB-Namen als Titel, änderbar im Spieldetail (7.6). Version 29 – Nachbesserung Stufe 12 (Entscheidung des Nutzers vom 16.09.2026): To-Do und Backlog sind mit der Bewertung gekoppelt – To-Do heißt `am_spielen`, Backlog `pausiert` (nie gestartete bleiben `nicht_gespielt`), durchgespielt/komplettiert/abgebrochen schließt den Eintrag automatisch statt als Vorschlag (neuer Abschnitt 5.5, Migration 0015); die Triage-Aktion „Spiele gerade" geht in „Auf To-Do" auf (sechs Aktionen); `PATCH /api/releases/:id/play-status` nur für den Status. Version 28: Stufe 12 – To-Do mit manueller Reihenfolge, Backlog mit Kandidaten und „nicht vorgesehen" (Migration 0014), Listen-Knöpfe im Spieldetail, Waisen beim Löschen eines Eintrags.*
...
- **Die Plattform wird vorgeschlagen und bleibt änderbar (Entscheidung des Nutzers vom
  ...
  „ohne Plattform" (`""`), das den Vorschlag vor dem Speichern ändert – **je Treffer**, wo eine
  Suche mehrere Treffer zeigt (`KandidatenListe` mit `mitPlattform`): Die Treffer nennen
  verschiedene Plattformen, deshalb steht am Treffer die konkrete neueste, nicht ein Platzhalter
  „neueste des Treffers" (Nachbesserung vom 15.09.2026). Ein Wunsch am Spiel sagt
...
**Rückfälle, nur wenn die Suche leer bleibt** (`kandidatenSuchen`, `src/sync/igdb.ts`; trifft 11 von 420 Titeln, kostet also fast nichts): erst der gekürzte Begriff – alles ab „ - ", „:" oder „/" fällt weg („CastleStorm - Complete Edition" → „CastleStorm", „Type:Rider" → „Type"); dann IGDBs Teilstringsuche über den Namen (`name ~ *"…"*`), der einzige Weg zu „That's You!" oder „We Were Here Too"; zuletzt dieselbe Teilstringsuche mit dem unbereinigten Titel, weil IGDB „OlliOlli2" ohne Leerzeichen schreibt. Was danach noch fehlt (aus der ersten Abnahme: „Poker Night at the Inventory 2", „TownsmenVR", „Wake-up Club"), kennt IGDB unter keiner Schreibweise – das bleibt „Gibt es bei IGDB nicht" oder eine Suche von Hand.
...
- Hat das Spiel Releases, muss sich mindestens eine Plattform des Kandidaten mit ihnen decken. Genau diese Prüfung löst „God of War (2018)" (PS4) und „God of War (2005)" (PS3) beide richtig auf.

**Nur PlayStation, ohne Ausnahme (Entscheidung des Nutzers vom 16.09.2026).** Die Anwendung kennt PS3, PS4, PS5 und Vita; ein IGDB-Eintrag ist nur dann ein Treffer, wenn er eine davon **nennt**. Bis Version 30 galt „fehlende Daten sind kein Gegenbeweis", und Einträge ohne Plattformangabe blieben zugelassen – so kam „Divinity: Original Sin II – Divine Edition", ein PC-Eintrag ohne Plattformdaten, als PS4-Wunsch durch. Seither: Jede IGDB-Abfrage – Volltextsuche, exakter Name, Teilstringsuche, auch die Rückfälle – trägt den Filter `platforms = (PS3, PS4, PS5, Vita)`, und `normalisiereTreffer` verwirft zusätzlich jeden Eintrag, dessen Plattformen keine der vier enthalten, auch beim Abruf nach Id (Verknüpfen, Wunsch aus `igdbId`, Nachpflege). Ein Eintrag, den IGDB ohne Plattform führt, ist damit nicht anlegbar – wer ihn braucht, wartet, bis IGDB die Plattform nachträgt, oder legt das Spiel von Hand an. Gemessen am 16.09.2026 gegen alle 470 Verknüpfungen: 469 nennen die eigene Plattform, 0 nur fremde, 1 ohne Angabe (der Divinity-Fall).
...
- **Titel beim Verknüpfen (Entscheidung des Nutzers vom 16.09.2026):** Ein Spiel **ohne Trophäenliste** – von Hand angelegt („Spiel anlegen") oder aus einem Wunsch – bekommt mit der Verknüpfung, automatisch wie manuell, den IGDB-Namen als Titel (`anzeigeTitel`, Schlüssel dazu); ein Spiel mit Trophäenliste behält den aus Sony normalisierten Titel, weil `title_name` die Referenz für alle Abgleiche ist. Nur beim Verknüpfen, nie beim Auffrischen: Der Titel bleibt im Spieldetail änderbar (die Überschrift ist ein Eingabefeld, `PATCH /api/games/:id`), und was der Nutzer umbenennt, überschreibt IGDB nicht mehr. Anlass: „Baldurs Gate 3" von Hand getippt, Verknüpfung richtig, Titel blieb falsch.
...
**Auffrischen.** `POST /api/igdb/auffrischen` holt für die 50 am längsten nicht aktualisierten verknüpften Spiele Wertung, Cover und Datum in **einer** Anfrage (`where id = (…)`) erneut. Kritikerwertungen ändern sich mit jeder neuen Rezension; seit Stufe 18 ruft der Cron den Schritt nachts mit einer **Frist von sieben Tagen** (`zumAuffrischen(n, mindestAlterTage)` – nur Spiele, deren `igdb_synced_at` älter ist oder fehlt), so ist jedes Spiel etwa wöchentlich dran und der Rest der Nacht bleibt ruhig; die Handschaltfläche nimmt ohne Frist die 50 ältesten.

**Physische Fassung aus IGDB (Stufe 14).** IGDB führt unter `external_games` Händler- und Store-Einträge je Spiel mit `media` (1 = digital, 2 = physisch) und `platform`. Gemessen am 14.09.2026 gegen 419 verknüpfte Spiele: 109 mit physischem Eintrag; **am 16.09.2026 gegen 469: 226** (IGDB trägt nach), 2 020 physische Einträge, alle von Quelle 20 (Amazon), **alle mit Plattform** – 694 davon fremde (Xbox, Switch, PC). Die Regel braucht deshalb keinen Rückfall: `physischePlattformen` (`src/domain/igdb.ts`) nennt die eigenen Plattformen, für die ein Eintrag mit Medium 2 existiert (PSVR/PSVR2 als PS4/PS5 wie überall), und `POST /api/igdb/physisch` setzt `physical_release_status = 'ja'` mit `physical_source = 'igdb'` für die Releases dieser Plattformen – **nur** von `unbekannt` aus: Fehlen sagt nichts, `nein` bleibt Handarbeit, ein bestehendes `ja` behält seine Quelle (Abschnitt 3). Ergebnis der Messung: 199 von 481 Releases.

Der Schritt arbeitet wie das Auffrischen: 50 verknüpfte Spiele je Aufruf in **einer** Anfrage (`physischNachIds`, eigene Feldliste `external_games.media,external_games.platform` – die Suche schleppt die Händlerliste nicht mit), Auswahl `igdb_id IS NOT NULL` mit mindestens einem `unbekannt`-Release, das nie oder vor mehr als 30 Tagen geprüft wurde (`physical_checked_at`). Alle `unbekannt`-Releases des Spiels werden gestempelt, auch ohne Treffer und auch bei leerer Antwort – sonst fragte der nächste Aufruf dasselbe Spiel wieder an. Die Frist macht ihn wiederholbar; seit Stufe 18 ruft der Cron ihn nachts, sobald Sync und Auffrischen nichts mehr zu tun haben (10.1). Antwort `{ status, angefragt, gesetzt, nochOffen, weiter }`; `GET /api/igdb/status` nennt `discBelegt` und `discOffen`. Der AWIN-Feed (7.3, Stufe 20) ist die zweite Quelle; beide dürfen ein `nein` des Nutzers nicht überschreiben. Abgelehnte Spiele stehen seit Stufe 11 in `v_ohne_igdb` mit `zustand = 'abgelehnt'` und in der Ansicht „Ohne Zuordnung" hinter einem Umschalter (8.3).
...
| `GET /api/trophy/v1/npCommunicationIds/{id}/trophyGroups/all/trophies` | Definition jeder Trophäe: Name, Beschreibung, Symbol, Art, versteckt, DLC-Gruppe | **Stufe 19b** |
...
| `GET /api/trophy/v1/users/me/trophySummary` | Trophäen-Level und Zähler je Stufe | Kennzahlen fürs Dashboard (Stufe 19) |
...
Die Sammlung umfasst 18 355 definierte Trophäen in 431 Listen (11 163 erspielt). Eine Stichprobe (PS3-Titel, 66 Trophäen) ergab 22,3 kB für die Definitionen, 9,1 kB für den eigenen Stand, 1,4 kB für die Gruppen.

Das ist für D1 unkritisch **beim Speichern** – die Datenbank liegt bei 2,6 MB von 5 GB. Kritisch wäre nur das Lesen, und das bleibt es nicht: Mit einem Index auf der Trophäenliste sind die Trophäen *eines* Spiels rund 50 gelesene Zeilen, nicht 18 000 (Abschnitt 2).

Beim Holen gilt derselbe Entwurf wie beim Sync (Entscheidung des Nutzers vom 21.09.2026):

1. **Einmal füllen, gestaffelt.** 431 Listen × 2 Abrufe = 862 Anfragen, verteilt über die nächtlichen Cron-Aufrufe (10.1) – wenige Nächte, Fortschritt in der Datenbank, begrenzte Arbeit je Aufruf.
...
3. **Rohdaten vor Normalisierung** wie in 7.1: erst die Antwort ablegen, dann auswerten.
...
**Rohdaten werden hier nicht abgelegt** (Entscheidung des Nutzers vom 22.09.2026) – die eine Ausnahme von der Regel aus 7.1, mit derselben Begründung wie bei IGDB: Beide Abrufe sind klein (zusammen rund 17 Anfragen), jederzeit neu abrufbar, und der Cron holt sie ohnehin regelmäßig. Die Normalisierung ist ein Feldkopieren plus Titelabgleich; wird sie später verbessert, genügt der nächste Lauf. Für die Trophäenseiten bleibt die Regel unberührt: Dort ist der Abruf teuer, die Normalisierung komplex, und die Rohdaten sind die einzige Aufzeichnung.

**Ein erkannter Kauf erledigt einen Wunsch nur bei „nur digital"** (Entscheidung des Nutzers vom 22.09.2026). Wer von Hand Besitz erfasst, weiß, was er getan hat – dort schließt der Eintrag Kauf und Wunsch ohne Rückfrage (Abschnitt 5). Meldet dagegen PSN einen digitalen Kauf, könnte der Wunsch der **Disc** gelten. Deshalb greift das Erledigen ausschließlich, wenn `release.physical_release_status = 'nein'` ist; bei `ja` oder `unbekannt` bleibt der Eintrag offen. Beim Bau am 22.09.2026 traf das auf **kein einziges** Release zu – die Regel wird erst mit dem AWIN-Feed (Stufe 20) oder von Hand gesetzten `nein` wirksam, und genau deshalb steht sie schon jetzt fest.
...
**Eine Betriebsart (seit Stufe 17d).** Der Scanner zeigt zu jedem Code seine Karte; „Überspringen" geht weiter, ohne etwas zu speichern. Bis dahin gab es daneben ein *Nur sammeln*, das erkannte Codes bloß wegschrieb und mit Tönen quittierte (hell = neu, zwei tiefe = schon bekannt, tief = Fehler). Es ist mit den offenen Scans entfallen (9.3): Was es sammelte, war später nicht mehr zuzuordnen.
...
Stufen 1, 4 und 5 laufen ohne Fremddaten. Der Barcode-Scan hängt damit **nicht** an der AWIN-Freigabe oder an eBay; Stufen 2 und 3 sind Verbesserungen, kein Fundament.
...
- `POST /api/scan` prüft Form (8–14 Ziffern) und Prüfziffer (EAN-8, UPC-A, EAN-13 – `src/domain/ean.ts`, fängt Tippfehler im Textfeld), löst Stufe 1 und 2 in einem Batch aus zwei Index-Lookups auf und hält jeden Code ohne Mapping in `unresolved_scan` fest (`scan_count` zählt hoch). Ein Mapping-Treffer bringt Titel, Cover und die Zahl der Exemplare („im Regal ×n") – Karte mit „Weiteres Exemplar", „Anderes Spiel", „Weiter".
...
- **Eine Fehllesung ist mit „Überspringen" erledigt.** Seit Stufe 17d wird ein nicht zugeordneter Code gar nicht mehr gespeichert, es gibt also nichts wegzuwerfen. Zuvor brauchte es dafür erst ein „Verwerfen" neben „Später" (21.09.2026), weil ein falsch gelesener Code sonst in der Liste stehen blieb – zweimal war das passiert (zwei vertauschte Ziffern, Prüfziffer trotzdem gültig).
...
**Titel beim Scannen, nicht erst nachts (Stufe 17c, Entscheidung des Nutzers vom 21.09.2026).** Die Sammelbetriebsart aus Stufe 17 hatte einen Konstruktionsfehler: Codes, die als „Später" liegen blieben, ließen sich am Schreibtisch nicht mehr zuordnen – die Disc stand längst wieder im Regal, und der nächtliche Job kam Stunden zu spät. Seit Stufe 17c fragt der Worker eBay **im Moment des Scannens** (`GET /api/scan/:ean/online`), und ein unbekannter Code fällt sofort auf, solange die Hülle in der Hand liegt.
...
- **Mehrheitsregel** (`mehrheitstreffer`, `src/domain/scan-titel.ts`): eBay liefert bis zu zehn Angebote je Code. Einzeln genügt eines, um danebenzugreifen – ein Bündel („Red Dead Redemption + GTA IV") oder ein Cross-Sell nennt ein fremdes Spiel. Ein Spiel gilt deshalb nur, wenn es in **mehr als der Hälfte** der eindeutigen Angebotstreffer steckt und öfter als jedes andere. Vor der Regel: drei Fehlgriffe.
...
- **Römische Zahlen II–IX werden auf Ziffern gebracht** (nachgereicht am 21.09.2026). In der ersten Messung fiel das durch – drei neue Fehlgriffe (Darksiders → Darksiders III, God of War → God of War III) –, aber die kamen aus der damals noch mitlaufenden Plattformziffer: Sobald „Sony PlayStation 3" keine blanke 3 mehr hinterließ, verschwanden sie. Ohne die Tabelle passt `Kingdom Come: Deliverance II` auf keines seiner eigenen Angebote, weil Verkäufer „2" schreiben, und das Grundspiel gewinnt – der erste Fehlgriff im echten Regal-Durchgang. `v` und `x` bleiben draußen: Sie sind auch gewöhnliche Buchstaben („Mega Man X"), und die Messung zeigte keinen Gewinn.
- **Rückhalt für den Sieger:** Ab drei Angeboten muss ihn mindestens zwei nennen. Ein Alleinkandidat wurde vorher bedingungslos genommen, auch wenn ihn nur eines von zehn Angeboten stützte – so wurde aus dem Bündel „GTA 4 + 5 Red Dead Redemption" ein Vorschlag für GTA. Das eigentliche Spiel stand dabei als `Red Dead Redemption (2010)` **und** `(2023)` in der Sammlung: Die Jahreszahl ist der Unterscheider des Nutzers, kein Verkäufer schreibt sie, also passte keine der beiden Fassungen – und das Feld war frei für den Zufallstreffer. Lieber kein Vorschlag als ein falscher; ohne Ziel zeigt die Oberfläche die Suche.

*Gemessen auf Release-Ebene (22.09.2026, beim Bau):* Von den 379 gelieferten Einträgen sind 303 Spiele – die übrigen 76 sind Streaming-Apps und Unbestimmtes und fallen heraus, bevor etwas gespeichert wird. **236 davon bekommen ein Release**, 67 bleiben ohne Zuordnung liegen (die Plattform muss mitpassen, deshalb weniger als die 284 des Titelabgleichs). Die Kaufliste ergibt **56 Kauf- und 160 PS+-Einträge**; 514 Einträge betreffen Spiele, die die Sammlung nicht kennt.
...
**Offen, nach Stufe 17 zu messen (Frage des Nutzers vom 16.09.2026, wie sich die Trefferquote erhöhen lässt):** Erster Messpunkt vom 17.09.2026 mit den drei Codes: eine einfache Websuche findet Hogwarts Legacy (eBay-Angebot) und Fallout 4 (Händlerseite), Darksiders nicht – 2 von 3. Der Scanner liefert mit `ean_mapping` und `unresolved_scan` die echten Codes des Regals als Messgrundlage. Danach werden die **eBay Browse API** (`item_summary/search?gtin=`, freier Developer-Account, Application-Token per Client-Credentials, Verkäufertext → `titelSchluessel` → Kandidaten aus der eigenen Sammlung als Vorschlag) und zum Vergleich upcitemdb im Scratchpad gegen 20–30 echte EANs gemessen; bei einer Quote deutlich über der Schwelle wird eBay als kleine Stufe „17b" die zweite Quelle der Kette. Weitere geprüfte Wege: mehrere UPC-Datenbanken kombiniert (Lebensmittel-lastig, wenig Gewinn), Redump.org und PriceCharting (spielespezifisch, aber ohne API bzw. bezahlt), GS1-Präfix (nennt nur den Verlag). **Ohne EAN:** ein Foto des Covers an ein Bildmodell (Titel und Plattform, rund 0,5 Cent je Foto) bleibt als Option für „im Laden prüfen" vermerkt – zweite externe Abhängigkeit mit Kosten, deshalb nicht Teil von 17. Klassische OCR auf dem Rücken und Rückwärts-Bildsuche scheiden aus (Logos, kein freies API).
...
### 9.3 Warum es keine offenen Scans mehr gibt (Stufe 17d)

Stufe 17 legte einen Barcode, der sich nicht sofort zuordnen ließ, als **offenen Scan** ab; Stufe 17b holte dazu nachts einen Titel bei `upcitemdb` und legte ihn in der Ansicht `/scans` zur Entscheidung vor. Gemessen an 56 Codes des PS3-Regals kannte die Quelle 35, davon führten 22 zu genau einem Spiel.

**Der Nutzer hat das am 21.09.2026 abgeschafft**, und die Begründung ist die eigentliche Lehre aus Stufe 17b: *„Es ist mir unmöglich, jetzt noch herauszufinden, welche Spiele es waren."* Ein Code ohne seine Hülle ist fast wertlos – die Disc steht längst wieder im Regal, und eine Zahl allein sagt nichts. Die Liste sammelte also genau das, womit man später nicht mehr arbeiten kann, und erzeugte Arbeit statt Nutzen. Seit der Scanner den Titel **im Moment des Scannens** liefert (9.2) und „Spiel anlegen" im selben Fenster steht, gibt es nichts mehr zu vertagen: Wer gerade nicht zuordnen kann, überspringt und scannt die Disc später erneut.

**Was damit entfallen ist:** „Später" und der Sammelmodus „Nur sammeln" im Scanner, die Ansicht `/scans` samt Einstellungen-Kachel, der Job `scripts/scans-aufloesen.mjs` mit `.github/workflows/scans.yml` und fünf Routen (`GET /api/scan/unresolved`, `GET /api/scan/ungeprueft`, `POST`/`DELETE /api/scan/:ean/vorschlag`, `DELETE /api/scan/unresolved/:ean`). `POST /api/scan` schreibt keinen Code mehr weg und antwortet mit `scans: 0`.

**Was geblieben ist:** `upcitemdb` als Rückfall hinter eBay, jetzt live im Worker (9.2) – die Quelle war nie das Problem, nur ihr Platz. Und die Tabelle `unresolved_scan` selbst: Sie steht leer, weil ein `DROP TABLE` zwei Deployments bräuchte (Abwärtskompatibilität, Abschnitt 15.2) und eine leere Tabelle niemanden stört. Sie bleibt in `EXPORT_TABELLEN`, bis sie fällt.

**Der Preis, bewusst bezahlt:** Es gibt keinen Notausgang mehr für eine Disc, die man gerade weder zuordnen noch anlegen will. Weil die Disc in der Hand liegt und ein erneuter Scan nichts kostet, ist das kein Verlust – wohl aber wäre es einer, wenn der Barcode das einzige Überbleibsel wäre. Genau das war der alte Zustand.
...
`wrangler.jsonc` trägt einen Cron `*/5 3-5 * * *` – alle fünf Minuten zwischen 03:00 und 05:59 UTC
(05–08 Uhr MESZ), 36 Aufrufe je Nacht, ein Eintrag von fünf, die der Free Tier je Konto erlaubt
(Entscheidung des Nutzers vom 19.09.2026: ein Nachtfenster statt ganztägig oder zweimal täglich).
...
Deshalb tut jeder Aufruf in dieser Reihenfolge genau **eines** und hört dann auf:

1. **Immer zuerst** `erschieneneFreigeben` (8.4) – nur SQL, kein CPU, protokolliert selbst.
1b. **Hängengebliebene Läufe abbrechen.** Ein Lauf mit `status = 'laufend'`, dessen letzter
...
4. Sonst, mit PSN-Zugang: **Spielzeit** (7.7) – eine Seite je Aufruf, täglich; der Stand steht als `datum:offset` in `app_setting`, ein abgeschlossener Tag ruht.
5. Sonst **Kaufliste** (7.7) – eine Seite je Aufruf, wöchentlich; der Stand hält Blätterung und die bisher gesehenen PS+-Releases. Erst nach der letzten Seite wird aufgeräumt, ein abgebrochener Lauf löscht nichts.
6. Sonst, mit IGDB-Zugang: `igdbAuffrischSchritt` mit Frist sieben Tage (7.6), wenn etwas fällig ist. Spiele, die IGDB nicht zurückgibt, werden trotzdem gestempelt – sonst wählt der nächste Aufruf dieselben und der Schritt dreht sich im Kreis (Befund vom 21.09.2026). Die Schritte 4 und 5 laufen in `try/catch`: Eine Ausnahme darf nicht den ganzen Aufruf reißen.
7. Sonst `igdbPhysischSchritt` (30-Tage-Frist in `DISC_OFFEN`), wenn etwas fällig ist.
8. Sonst **alte Rohantworten aufräumen** (Stufe 18d): `psn_raw_response` behält die Seiten der jüngsten **drei** Läufe, die überhaupt Seiten haben, und **alles noch nicht Normalisierte** – unabhängig vom Alter, denn das ist unerledigte Arbeit, kein Archiv. Ein einzelnes `DELETE` über den Index auf `sync_run_id`, also die leichteste Arbeit der Reihenfolge; sie steht ganz hinten und belegt einen Aufruf, der sonst „nichts" täte. Kein `game_event`: Es ändert sich kein Spiel und keine Entscheidung, nur Fremddaten, die jederzeit neu abrufbar sind (8.5). **Warum überhaupt:** Der Zweck der Rohablage ist eine ohne PSN wiederholbare Normalisierung (7.1), nicht ein Archiv jeder Nacht. Am 23.09.2026 waren **2,31 von 3,26 MB** der Datenbank Rohantworten – fünf Seiten je Nacht, immer dieselben 431 Titel, 263 KiB täglich –, und nichts löschte sie je. Über `d1 export` wandern sie zusätzlich in jede wöchentliche Sicherung und damit dauerhaft in die Historie des privaten Backup-Repositorys (14.2).
9. Sonst nichts.
...
brauchen bei fünfzig je Aufruf **acht** Aufrufe. Das Fenster trägt das (elf Sync, zwei Spielzeit,
acht IGDB, einer Aufräumen = 22 von 36). Es trägt es **nicht** mehr, wenn eine Kaufliste-Nacht
zusammenfällt: elf + zwei + fünfzehn + acht + einer sind **37**. Diesmal liegen die beiden Termine
einen Tag auseinander (29. und 30.09.), das ist Zufall und keine Eigenschaft des Entwurfs. Wer dem
Cron einen Schritt hinzufügt (Stufe 20), rechnet gegen diese 37, nicht gegen die siebzehn, die
hier früher standen. Ein Leerlauf-Aufruf liest gemessen
2 241 Zeilen (`test/lesekosten.spec.ts`; 2 165 vor Stufe 18d, der Aufräumschritt kostet 76), die Nacht
damit rund 80 000 – weit unter dem Tagesbudget.
...
und der Verlauf fasst zwanzig Einträge. Eine volle Nacht sind elf Sync-Aufrufe, zwei für die
Spielzeit, **fünfzehn für die Kaufliste**, ein bis zwei für IGDB, einer fürs Aufräumen und eine
verdichtete Leerlaufzeile – **einunddreißig**. Hier stand bis zum 24.09.2026 „siebzehn", mit je
einem Aufruf für Spielzeit und Besitz gerechnet, während 7.7 für dieselben Schritte zwei und
fünfzehn nennt. **Zwanzig Einträge fassen eine Kaufliste-Nacht damit nicht:** Aufgehoben werden
die jüngsten zwanzig, also fallen die ältesten elf bis zwölf weg – und das sind die Sync-Zeilen. In
den übrigen sechs Nächten der Woche reicht es. Der Sync ist dabei nicht unsichtbar, er steht als
`letzterAutomatischerLauf` neben dem Verlauf; unsichtbar wird sein *Fortschritt*, also gerade das,
was Stufe 18d sichtbar machen sollte. Wie das zu lösen ist – mehr Einträge, Verdichtung auch
gleichartiger Arbeitsaufrufe oder eine Zeile je Schritt statt je Aufruf –, **ist in einer späteren
Stufe zu entscheiden**; die nächste betroffene Nacht ist die zum 30.09.2026 (`psn_besitz_stand`
steht auf dem 23.09., Frist sieben Tage). Ein **Fehler ist kein Leerlauf** und wird nie verdichtet; das
Lebenszeichen bleibt trotzdem stehen, weil die verdichtete Zeile die Zeit des jüngsten Aufrufs
trägt. `GET /api/sync/status` nennt `letzterAutomatischerLauf`; die
...
| Sammlung | 1 | **Kacheln oder Zeilen, umschaltbar** (Stufe 19), Chip-Filter, Suche hinter der Lupe. Der Besitz ist dort **nur Anzeige**: Disc- und Wolken-/PS-Plus-Zeichen, hell wenn vorhanden, im `--aus`-Ton wenn nicht. Die Schnellerfassung je Release („+ Disc", „+ digital") mit Rückgängig **entfällt mit Stufe 19** (Entscheidung des Nutzers vom 23.09.2026): Sie war mit der Klickzahl bei der Ersterfassung des Regals begründet, die ist abgeschlossen, und erfasst wird seither über den Scanner (Abschnitt 9) und im Spieldetail – beide tragen `absichtenErledigen` samt Rückgängig, es geht also kein Verhalten verloren, nur ein Weg. „Spiel anlegen" für Titel ohne Trophäenliste – seit Stufe 17 mit IGDB-Suche im selben Formular: ein Treffer legt an, verknüpft und bringt Cover und Wertung mit, die Plattform als Dropdown am Treffer; „Ohne IGDB-Eintrag anlegen" bleibt für Titel, die IGDB nicht kennt. Seit Stufe 9 das IGDB-Cover im Hochformat; das Trophäensymbol bleibt Rückfall, solange eine Verknüpfung fehlt |
| Spieldetail | 1, 2, 7 | *(seit Stufe 18c je Release „Spielzeit: 13 h · 5-mal gestartet · zuletzt …", bei PS3 und Vita dauerhaft „unbekannt" mit dem Zusatz, dass PSN dort keine liefert; digitale Berechtigungen nennen ihre Herkunft, von PSN erkannte ohne Löschkreuz; ab 19b die Trophäenliste mit erspielt/offen, 7.7)* Releases, Exemplare (Zustand, Anleitung, Kaufdatum, Preis, EAN, Notiz), digitale Berechtigungen; je Release „Trophäen (Sony)" und „Eigene Bewertung" (Status, Bewertung 1–10, Begonnen/Beendet, Notiz) nebeneinander, nie verrechnet (die Bewertung zieht seit 5.5 To-Do und Backlog nach); Preisverlauf je Kanal; Release hinzufügen und löschen, Spiel löschen. Block „IGDB" (seit Stufe 9): Kritikerwertung mit Anzahl und Quelle, Erscheinungsdatum und Status, Herkunft der Verknüpfung, Link zu igdb.com; „Anderen Eintrag wählen", „Verknüpfung lösen", ohne Verknüpfung Suche und „Gibt es bei IGDB nicht". Block „Verlauf" (seit Stufe 16, 8.5): die letzten 20 Ereignisse des Spiels mit Zeit, Quelle als Pille und Satz, „ältere anzeigen" |
...
| Lücken | 3 | `/luecken`, in der Leiste (seit Stufe 14): digital gespielt, Disc-Fassung belegt, nicht im Regal – Cover, Plattform, Fortschritt, Platin, eigener Status, Gebrauchtpreis („unbekannt" bis Stufe 20). Knopf „physisch nicht gewünscht" mit Rückgängig; verworfene standardmäßig ausgeblendet, per Umschalter sichtbar mit „wieder als Lücke zeigen". Darunter zugeklappt „Disc-Fassung unbekannt (n)" mit „Disc gibt es" / „gibt es nicht" / „physisch nicht gewünscht" je Zeile (5.3, Block B). Im Spieldetail je Release ein Dropdown ja/nein/unbekannt mit Quelle (aus IGDB, von Hand) und das Feld PSN-Produkt-Id; in den Einstellungen unter IGDB der Knopf „Disc-Fassungen prüfen" mit Zählern (7.6) |
| Wunschliste | 4, 11 | Sortierung Favoriten zuerst → Wertung (Standard), Wertung, Titel, Erscheinungsdatum, zuletzt angelegt (5.2); Suchfeld (Titel); Filter nur Favoriten, Plattformen (Mehrfachauswahl) und „ohne Plattform" – zum Nachpflegen; erledigte und verworfene standardmäßig ausgeblendet; je Eintrag Cover, Kritikerwertung, Jahr, Favorit-Stern, Plattform als **Kennzeichen**, das auf Tippen zum Auswahlfeld wird (hängt den Wunsch um), erledigt/verworfen/wieder öffnen, entfernen; die **Notiz wird seit Stufe 19 nicht mehr angezeigt** – sie bleibt in Datenmodell, API und Spieldetail (Rückmeldung des Nutzers vom 22.09.2026); Erscheinungsdatum statt Preis bei angekündigten Titeln. „Wunsch hinzufügen" über die IGDB-Suche, Plattform-Dropdown an jedem Treffer (vorbelegt mit dessen neuester, „ohne Plattform" wählbar); Freitext nur über „Ohne IGDB-Eintrag übernehmen" nach einer Suche; Rückgängig direkt nach dem Anlegen. „Liste importieren" sitzt seit Stufe 19 hinter demselben Pluszeichen wie „Wunsch hinzufügen". Karte, Filter und Schreibzugriffe teilt sie sich seit Stufe 12 mit To-Do und Backlog (`frontend/src/Absichten.tsx`: `usePlanListe`, `PlanKarte`). Im Spieldetail ein Block „Listen" (bis Stufe 12 „Wunschliste"): offene Absichten als Pillen, auf die Wunschliste setzen (Plattform vorbelegt mit der neuesten des Spiels, auch eine ohne Release oder „ohne Plattform" wählbar), je Release „Auf To-Do" / „Ins Backlog" (Abschnitt 5) |
| To-Do | 5a | `/todo`, Reiter „To-Do \| Backlog": eine Spalte in eigener Reihenfolge, die ganze Karte zieht (Maus ab 6 px, Finger nach 200 ms, Tastatur; `@dnd-kit`), jede Umsortierung sofort gespeichert; Filter nur Favoriten und „auch erledigte", keine Sortierwahl; je Kachel wie die Wunschliste, aber statt erledigt/verworfen „ins Backlog" (setzt `pausiert`), „durchgespielt", „abgebrochen" (5.5) |
...
| Kaufliste | 6, 10 | `/kaufliste`, in der Leiste (seit Stufe 15): sortiert und gefiltert wie die Wunschliste (5.2), dieselbe Kachel mit Herkunft („aus Lücke", „aus Wunsch", „von Hand") und „im Besitz" bei Altfällen; erledigt/verworfen/wieder öffnen, entfernen. Darunter **Kandidaten** aus `v_kaufkandidaten` in zwei aufklappbaren Blöcken „aus Lücken (n)" – mit „auf die Kaufliste" und „physisch nicht gewünscht" (5.3) – und „aus Wünschen (n)" mit „auf die Kaufliste" (Kopie, Favorit kommt mit); Rückgängig nach jeder Übernahme und nach dem Verwerfen. Die Wunsch-Kachel trägt denselben Knopf beziehungsweise den Link „auf der Kaufliste"; im Spieldetail je Release „Auf die Kaufliste" (gesperrt bei offenem Kaufeintrag). Gebrauchtpreis „unbekannt" bis Stufe 20; ein Link auf „Erscheint bald", sobald es dort etwas gibt |
...
| Änderungen | – | `/aenderungen` (seit Stufe 16, Werkzeug in den Einstellungen, 8.5): alle Ereignisse neueste zuerst, Quelle als Dropdown (alle / du / PSN-Sync / IGDB / Import …), je Zeile Zeit, Quelle, Titel mit Link ins Spieldetail (ohne Link, wenn das Spiel gelöscht ist) und Satz; „ältere laden" per Keyset. Nur lesend – korrigiert wird dort, wo die Änderung entstand |
| Scannen | 1 | `/scannen`, in der Leiste (seit Stufe 17): Serienerfassung nach Abschnitt 9 in drei festen Zonen – Kamerabild (auf dem Handy auf gut ein Drittel der Höhe begrenzt, „Kamera wechseln" bei mehreren, Pille „Fallback" ohne nativen Detektor), Textfeld „EAN eintippen" als Notnagel, Ergebniskarte mit fester Mindesthöhe. Karte je Fall: Mapping-Treffer mit Cover, Titel, „im Regal ×n" und „Weiteres Exemplar" / „Anderes Spiel" / „Weiter"; „Verwerfen" wirft eine Fehllesung sofort weg (mit Rückfrage, seit 21.09.2026); sonst seit Stufe 17c zuerst der **Titelvorschlag von eBay** („eBay: …", Kandidaten der Sammlung mit „Erfassen (PS3)" je Release, das Ziel mit der Pille „Vorschlag"), nachgeladen nach der Karte, damit die Anzeige nicht wartet; darunter wie bisher die Suche in der Sammlung (Knopf je Release, Dropdown „andere Plattform") und „Spiel anlegen" wie in der Sammlung, „Später"; nach dem Zuordnen „Disc erfasst: Titel (PS4)" mit erledigten Absichten, „ins Backlog übernehmen", Rückgängig und dem Sitzungszähler – die Erkennung läuft sofort weiter. `?ean=` (aus den Einstellungen) löst sofort auf, die Kamera bleibt aus |
| Einstellungen | – | NPSSO, Sync (letzter Lauf mit „von Hand"/„automatisch"), Block „Automatik" (seit Stufe 18: Fenster, was nachts geschieht, letzter automatischer Abruf; 10.1), IGDB (Zugangsdaten ja/nein, Zähler verknüpft/zur Prüfung/nicht gesucht/abgelehnt, „Abgleich starten" mit Fortschritt, „Metadaten auffrischen", „Offene erneut suchen"), offene Scans (seit Stufe 17: EAN, wie oft, zuletzt, „zuordnen" → `/scannen?ean=`, „verwerfen"; nur bei Anzahl > 0), Abweichungen (seit Stufe 6, mit Link ins Spieldetail), Export und Backup-Status |

**Navigation:** Auf dem Handy eine Leiste am unteren Rand mit **vier Symbolen ohne Text** – Start, Sammlung, Wunschliste, To-Do (Stufe 19). Der aktive Eintrag ist golden und trägt einen Punkt; was ein Symbol bedeutet, sagt sein `aria-label`. **Wie es dazu kam:** Die Leiste wuchs bis Stufe 17 auf **sieben** Einträge mit Text – ab dem fünften stand das Symbol über dem Text, ab dem sechsten schrumpfte die Schrift auf 0,7 rem, ab dem siebten auf 0,6 rem mit engerem Buchstabenabstand, damit sie auf 320 px passte. Das war ausdrücklich eine Notlösung. Drei Einträge sind deshalb umgezogen (Entscheidung des Nutzers vom 23.09.2026): **Kaufliste und Lücken sind Reiter der Wunschliste**, wie das Backlog neben To-Do (`/wunschliste`, `/kaufliste`, `/luecken`; der Leisteneintrag bleibt auf allen dreien markiert); **Scannen** ist eine Aktion in der Kopfzeile der Sammlung; **Einstellungen** sind das Zahnrad in der Kopfzeile. Die Wunschliste kam mit Stufe 10 in die Leiste (Entscheidung des Nutzers vom 14.09.2026): Sie wird oft bedient, und ein Umweg über die Einstellungen wäre für die häufigste Liste der falsche Platz. **Am Desktop dieselbe Navigation als Seitenleiste** – dort **mit** Text und mit den Reitern als eingerückten Unterpunkten, weil der Platz da ist (Wunsch des Nutzers vom 23.09.2026); das Zahnrad steht unten in der Leiste statt in der Kopfzeile. Die Prüfliste und der Import sind keine Dauernavigation, sondern werden von der Startseite aus aufgerufen, solange sie offene Posten haben – mit Anzahl als Kennzeichen.

Routing über `react-router-dom` mit echten Pfaden (`/sammlung`, `/spiel/:id`, `/wunschliste`, `/pruefliste`, `/einstellungen`, `/zuordnung`, `/igdb`, `/pruefen`, `/trophaeen`, seit Stufe 11 `/import`, `/import/:id`, `/ohne-zuordnung`, seit Stufe 12 `/todo`, `/backlog`, seit Stufe 14 `/luecken`, seit Stufe 15 `/kaufliste`, `/erscheint-bald`, seit Stufe 16 `/aenderungen`, seit Stufe 17 `/scannen`, seit Stufe 19 `/start`); der SPA-Fallback des Workers (15.2) liefert für jeden Pfad die `index.html`. Die Leiste trägt seit Stufe 19 vier Einträge (Start, Sammlung, Wunschliste, To-Do); `/` leitet auf `/start` um, ebenso jeder unbekannte Pfad. Zuordnung, IGDB-Zuordnung, Sammlung prüfen, Trophäen, Wunschliste importieren, Ohne Zuordnung, Erscheint bald und Änderungen hängen als Werkzeuge an den Einstellungen, der Import zusätzlich als Link auf der Wunschliste. Die Sammlungsfilter liegen in der URL, damit „Zurück" aus dem Spieldetail den Stand wiederherstellt.

Bis es das Dashboard gibt, übernimmt ein Hinweisblock dessen Rolle – seit Stufe 19 auf der **Startseite** (`/start`) statt oben in der Sammlung, damit die Sammlung mit ihrem Inhalt beginnt und die offenen Posten dort stehen, wo die App startet: offene Prüfliste, `unentschieden`-Einträge (auch bei leerer Prüfliste), nicht zugeordnete Trophäenlisten, seit Stufe 8 eine **überfällige Sicherung** (mehr als acht Tage oder noch nie, siehe 14.2) seit Stufe 9 Spiele, die noch nicht bei IGDB gesucht wurden oder auf die IGDB-Zuordnung warten, seit Stufe 11 einen Import mit offenen Zeilen sowie Freitext-Einträge ohne Spiel und seit Stufe 12 die Backlog-Kandidaten (5.4) – jeweils nur bei Anzahl > 0 und mit Link. Seit Stufe 18 außerdem den **abgelaufenen PlayStation-Zugang** („der nächtliche Abruf steht still", der deutliche Hinweis aus 7.1, bis dahin nur in den Einstellungen) und einen **fehlgeschlagenen automatischen Abruf** jünger als 24 Stunden mit seiner Meldung – der Nachtlauf hat keinen Zuschauer, sein Fehler muss am Morgen hier stehen. Ist **nichts** offen, schweigt die Seite nicht, sondern sagt es – eine leere Startseite sähe nach einem Fehler aus, wo „nichts zu tun" die gute Nachricht ist. Beim Dashboard-Bau (Stufe 19a) wird aus dieser Seite das Dashboard.
...
- Das **Platin-Zeichen ist dreiwertig wie die Daten** (93 von 431 Titeln haben gar keine Platin-Trophäe): gefüllter Pokal = erspielt, Umriss im `--aus`-Ton = offen, **gar nichts** = nicht vorgesehen. Ein durchgestrichenes oder graues Zeichen an dieser Stelle wäre eine vierte Aussage, die es nicht gibt.
...
**PWA (Stufe 18, `vite-plugin-pwa`, Workbox `generateSW`):** Manifest (`Trophytracker`, `standalone`, `lang de`, Symbole 64/192/512, maskable, Apple-Touch) und Service Worker mit `autoUpdate`. **`theme_color` ist seit Stufe 19 die Grundfarbe der Anwendung (`#12151c`), `background_color` bleibt der Symbolhintergrund (`#1f2633`).** Vorher war beides dieselbe Konstante, und am Bildschirmfoto des Nutzers war die Kante messbar: Statusleiste rgb(31,38,51), Seite darunter rgb(18,21,28). Strategien:

- **Navigationen Network-First**, Cache `seite`. `index.html` liegt **bewusst nicht im Precache**: Cloudflare Access prüft Navigationen; käme die App-Hülle aus dem Cache, scheiterte bei abgelaufener Sitzung jede API-Anfrage am Redirect auf eine fremde Origin, und ein Neuladen änderte nichts – Endlosschleife ohne Anmeldung. Online läuft die Anmeldung wie ohne Service Worker, offline kommt die zuletzt geladene Seite aus dem Cache und die gehashten Assets aus dem Precache. Nebeneffekt: Ein normales Neuladen bringt nach einem Deploy die neue Fassung.
- **Alle `GET /api/*` außer `/api/export/*` Network-First**, Cache `api` (400 Einträge, 30 Tage) – alle Leseansichten offline lesbar (Entscheidung des Nutzers vom 19.09.2026, statt nur Sammlung und Spieldetail). Ohne Netz-Timeout: Ein Timeout zeigte bei langsamem Netz Altstände neben gerade gespeicherten Änderungen.
- **Nur eigene Antworten in den Cache:** Für `seite` und `api` speichert ein `cacheWillUpdate` nur `response.ok && !response.redirected` von derselben Origin. Gemessen am 19.09.2026: Ohne Sitzung antwortet Access mit `302` auf `trophytracker.cloudflareaccess.com`, nach Folgen eine `200`-HTML-Seite fremder Origin – die läge sonst als „Sammlung" im Cache.
- **Cover von `images.igdb.com` Cache-First** (800 Einträge, 60 Tage, auch opaque), **WASM des ZXing-Lesers Cache-First** zur Laufzeit statt im Precache (1,1 MB, nur für Geräte ohne nativen `BarcodeDetector`).
- **Offline-Balken** oben in jeder Ansicht („Offline – du siehst den zuletzt geladenen Stand. Änderungen sind erst wieder online möglich.", `frontend/src/Offline.tsx`). Kein Offline-Schreiben, keine Warteschlange. `anfrage` meldet eine Nicht-JSON-Antwort als „Anmeldung abgelaufen – bitte die Seite neu laden" statt still `{}` zurückzugeben. Im Headless-Browser gegen `wrangler dev` geprüft: Service Worker aktiv, Caches `seite`/`api`/`cover` gefüllt, offline dieselbe Sammlung mit Balken.

**Gestaltung – die Linie „Vitrine" (Stufe 19).** Ein dunkler Schaukasten, in dem die Cover die einzigen bunten Flächen sind; die Oberfläche bleibt still, **Farbe ist ausschliesslich Information**. Die Werte stehen als Tokens in `frontend/src/tokens.css` und nirgends sonst – vor Stufe 19 lagen Farben als `rgba(128,128,128,…)` verstreut in 1 286 Zeilen `App.css`.

- **Nur dunkel** (Entscheidung des Nutzers vom 23.09.2026): `color-scheme: dark`, ein einziger Tokensatz, keine helle Fassung zu pflegen.
- **Gold `#e8b923` ist der einzige Akzent** und wird sparsam vergeben: aktiver Leisteneintrag, Favoritenstern, 100 %, erspieltes Platin. Es ist die Farbe des App-Symbols.
- **Plattformen** tragen einen eigenen Farbcode – PS3 rot, PS4 hellblau, PS5 dunkelblau, Vita grün – als Textfarbe und Rand eines Kennzeichens, nie als Fläche. Das ist die Zuordnung des Nutzers, keine Marke; der Name bleibt Text (siehe unten).
- **Zustände** haben je eine Farbe, die als Punkt neben dem Wort steht: am Spielen **grün**, komplettiert **gold**, durchgespielt blau, pausiert graublau, abgebrochen rot, nicht gespielt neutral, unentschieden violett. Komplettiert ist gold, weil damit Prozentzahl, Pokal, Punkt und Fortschrittsbalken eines komplettierten Spiels **dieselbe** Farbe tragen – die Auszeichnung wird eine Aussage statt vier verstreuter Zeichen (Entscheidung des Nutzers vom 23.09.2026).
- **`--aus` ist ein Ton für alles, was fehlt**: keine Disc, kein Download, offenes Platin. Vorhanden ist hell, nicht vorhanden ist dunkel – eine Regel statt drei Sonderfällen. Bewusst leise, aber erkennbar.
- **Der Fortschrittsbalken trägt die Zustandsfarbe.** Ein Element leistet damit beides; die Kachel bekommt keinen farbigen Rand, weil eine eingefärbte Karte je Zustand zu laut wäre.
- **Schrift:** `Saira` (Google Fonts) für Titel, Plattform-Kennzeichen und Zahlen (Tabellenziffern), die System-Schrift des Geräts für Fliesstext und Bedienelemente. Aus zehn verglichenen gewählt (Nutzer, 23.09.2026). Fällt der Abruf aus – offline, geblockt –, übernimmt die System-Schrift, ohne dass etwas bricht; der Service Worker legt Stylesheet und Schnitt dauerhaft ab. **Eine an PlayStation angelehnte Schrift kommt nicht in Frage** (Wunsch des Nutzers, abgelehnt): Sonys Hausschrift ist lizenziert, eine Nachahmung fiele unter dieselbe Markenregel wie die Logos.
- **Zeichen:** ein eigener Satz als Inline-SVG (`frontend/src/Symbole.tsx`), 24 × 24, `currentColor`, eine Strichstärke. Keine Icon-Bibliothek – bei einer fremden müsste jedes Zeichen einzeln gegen die Markenregel geprüft werden, und der Satz ist klein genug, um ihn zu zeichnen. Gefüllt wird nur, wo die Füllung eine Aussage ist (erspieltes Platin, gesetzter Favorit). PS Plus ist ein **dickes Kreuz als Umriss**, ein gekaufter Download eine Wolke; liegt beides vor, gewinnt der Kauf – dieselbe Regel wie im Datenmodell (7.7).
- **Suche und Filter:** Die Suche klappt erst auf Tippen auf die Lupe auf („nimmt immer oben sehr viel Platz weg", Rückmeldung vom 22.09.2026); ein gesetzter Begriff hält sie offen. Filter sind **Chips**, und zwar in **zwei Ebenen**: Sichtbar ist nur, was gesetzt ist, davor ein Knopf „Filter" mit der Anzahl, der eine Tafel mit allen Gruppen aufklappt (jede mit Namen, Chips umbrechend). Die erste Fassung legte alle nebeneinander – in der Sammlung 23 Stück, und die Rückmeldung war „zu lang" (Nutzer, 24.09.2026). Dieselbe Gestalt in Sammlung und Listen; vorher waren es dort Dropdowns und hier Kästchen.
- **Kacheln oder Zeilen**, je Liste getrennt umschaltbar; die Wahl liegt im Gerät (`localStorage`), nicht am Konto: Auf dem Handy taugt die Zeile, am Schreibtisch die Kachel. To-Do und die Lückenansicht bleiben Listen – ihre Reihenfolge beziehungsweise ihre Blöcke sind der Inhalt.
- **Die Plattform eines Listeneintrags** steht als Kennzeichen da, nicht als Dropdown (Wunsch des Nutzers vom 24.09.2026). Ein Tipp darauf macht sie zum Auswahlfeld – das Umhängen auf eine andere Plattform bleibt möglich (5.2), ohne dass ein Auswahlfeld die Zeile beherrscht.
- **`viewport-fit=cover`:** Die Seite reicht unter Status- und Systemleiste, die Kopfzeile füllt den oberen Rand mit ihrer eigenen Fläche, die Leiste den unteren. Ohne das sind alle `env(safe-area-inset-*)` null und das Betriebssystem malt den Streifen selbst – auf dem Handy des Nutzers als heller Strich unter der Leiste (24.09.2026).
- **Zurück nach oben:** ein runder Knopf über der Leiste ab 600 px Scrolltiefe, in jeder Ansicht (Wunsch des Nutzers vom 22.09.2026; bei 431 Titeln ist der Rückweg sonst Arbeit).

**Symbol und Gestaltung:** Das App-Symbol ist ein eigener Pokal in einem **Fortschrittsring** (`frontend/public/icon.svg`, PNGs aus `@vite-pwa/assets-generator`, `pwa-assets.config.ts`). Der Ring ist zu 87 % geschlossen – eine Sammlung ist nie ganz fertig. Bis Stufe 19 stand dort der Pokal allein; der Nutzer wählte den Ring am 24.09.2026 aus zehn Entwürfen, weil das Symbol damit auch etwas **anzeigt**. Zwei Farben, kein Text. Der Ringaußenrand liegt bei Radius 189 von 256 und damit innerhalb der maskierbaren Schutzzone (204,8), sodass Androids Rundschnitt nichts abschneidet; der Pokal steht auf 0,68 seiner Originalgröße – gemessen gegen 64 px, kleiner verlor er im Ring an Kraft. **Regel für jede Gestaltungsentscheidung (Nutzer, 19.09.2026): keine PlayStation-Logos, -Symbole oder -Schriftzüge, auch nicht angedeutet oder abstrahiert** – geschützte Marken, das Repository ist öffentlich. Plattformnamen als Text („PS4", „Vita") sind Daten, kein Logo.
...
**Umfang von `backup.json`:** `{ exportiertAm, schemaVersion, tabellen }` mit den **15 Fachtabellen** `game`, `release`, `physical_copy`, `digital_entitlement`, `trophy_progress`, `play_status`, `plan_entry`, `review_queue`, `ean_mapping`, `unresolved_scan`, `market_offer`, `price_snapshot`, `app_setting`, `psn_sync_run`, seit Stufe 16 `game_event` (8.5 – Entscheidungen des Nutzers, die kein Sync zurückbringt). Nicht dabei sind `psn_credentials` (Chiffrate und IVs), `psn_raw_response` (Rohdaten, gross, im SQL-Dump ohnehin enthalten) und `d1_migrations` (Wranglers Buchführung). `schemaVersion` ist der Name der höchsten angewendeten Migration. Die Liste steht als `EXPORT_TABELLEN` in `src/db/export.ts`; ein Test hält fest, dass **jede** Tabelle der Datenbank entweder exportiert oder ausdrücklich ausgenommen ist – eine Tabelle aus einer künftigen Migration kann so nicht stillschweigend ungesichert mitfahren.
...
Trophäen-Sync erzeugt einige hundert Anfragen, nicht hunderttausend. Die 36 Cron-Aufrufe je
Nacht (10.1) zählen als Anfragen und lesen im Leerlauf rund 80 000 Zeilen; ein Cron Trigger von
fünf erlaubten.
...
| 17b | Offene Scans zuordnen: Titel aus einer EAN-Quelle über einen GitHub-Job (Migration 0020), Abgleich mit der Sammlung beim Lesen, Ansicht `/scans` in Blöcken | Ein gescanntes Regal lässt sich am Schreibtisch abarbeiten; gemessen 22 von 56 Codes eindeutig (Entscheidungen des Nutzers vom 18.09.2026). eBay als zweite Quelle vertagt, bis der Entwicklerzugang freigegeben ist – mit Stufe 17c eingelöst. **Abgenommen 19.09.2026**: 34 Discs per Barcode erfasst, 22 Codes ohne Titel bleiben offen bis eBay |
| 17 | Barcode-Scan mit Auflösungskette (ohne Feed): `/scannen` mit Handy-Kamera und Laptop-Webcam, Polyfill `barcode-detector`, Suche in der Sammlung, „Spiel anlegen", offene Scans; keine Migration | Komfort bei Erfassung – erster neuer Schreiber im Protokoll („per Barcode"); Trefferquote externer Quellen nach dem Regal-Erfassen gegen echte Codes gemessen (9.2, 9.3). **Abgenommen 19.09.2026** mit dem PS3-Regal |
| 17c | eBay als EAN-Quelle, live beim Scannen (9.2): Kette `ean_mapping` → `market_offer` → eBay → upcitemdb, Mehrheitsregel und Ballast-vor-Ziffern im Abgleich; keine Migration | Ein unbekannter Code fällt beim Scannen auf, nicht Wochen später (Entscheidung des Nutzers vom 21.09.2026, nach Freigabe des eBay-Entwicklerzugangs). Gemessen über alle 42 echten Codes: 37 richtig, kein Fehlgriff. **Abgenommen 21.09.2026** nach dem ersten Regal-Durchgang – acht Discs erfasst, jeder Code lieferte einen Vorschlag; die zwei dabei gefundenen Fehler (römische Fortsetzungsnummer, Alleinkandidat aus einem Bündel) sind behoben |
| 17d | Offene Scans abgeschafft: „Später", Sammelmodus, `/scans` und der nächtliche Job entfallen; upcitemdb wird Live-Rückfall im Worker (9.3) | Ein Code ohne seine Hülle war später nicht mehr zuzuordnen – die Liste erzeugte Arbeit statt Nutzen (Entscheidung des Nutzers vom 21.09.2026). Keine Migration; `unresolved_scan` bleibt leer stehen. **Abgenommen 21.09.2026** |
| 18 | Cron Trigger `*/5 3-5 * * *` mit einem Schritt je Aufruf (10.1, Migration 0021), PWA mit Network-First-Seite und Offline-Lesezugriff auf alle Leseansichten (13) | Automatik und Komfort – der Cron-Sync ist der erste rein automatische Schreiber, den das Protokoll zeigt; Use Case 11 damit vollständig (täglicher Statuswechsel). Entscheidungen des Nutzers vom 19.09.2026: Nachtfenster, ein Versuch je Nacht, Abbruch hängender Läufe, alle Leseansichten offline, eigenes Pokal-Symbol ohne Marken. **Abgenommen 22.09.2026**: PWA installiert und offline geprüft, Nachtläufe am 20., 21. und 22.09. mit je 431 Titeln, in der Nacht zum 22.09. zusätzlich 368 aufgefrischte Spiele. Der IGDB-Schritt hatte zwei Nächte geschwiegen; Stufe 18b hat ihn repariert und nachprüfbar gemacht |
| 18c | Spielzeit und digitaler Besitz aus PSN (7.7): `gamelist/v2` für `playDuration`/`playCount`/erstes und letztes Spieldatum, Kaufliste per GraphQL mit `subscriptionService` (gekauft / PS+); Abgleich über `titelSchluessel`, nur ergänzend, kein Import; Herkunftsspalte an `digital_entitlement`; Migration | Spielzeit im Spieldetail und als Sortierkriterium (5.2), digitaler Besitz endlich ohne Handarbeit. Gemessen am 21.09.2026: 284 von 379 gespielten Titeln und 247 von 730 Käufen passen eindeutig. Entschieden am 22.09.2026: Titel ohne Spielzeit ans Listenende, `kauf` schlägt `plus`, die PS+-Seite ist eine Momentaufnahme und wird je Lauf als Ganzes ersetzt; ein erkannter Kauf erledigt einen Wunsch nur bei „nur digital" (7.7). **Abgenommen 22.09.2026** (Migration 0023); gemessen: 236 Releases bekommen Spielzeit, 56 Kauf- und 160 PS+-Einträge entstehen |
| 18d | Der Cron wird les- und aufräumbar (10.1): Leerlauf-Aufrufe im Verlauf verdichtet und zwanzig statt fünf Einträge, alte `psn_raw_response` als letzter Schritt der Nacht gelöscht (die jüngsten drei Läufe und alles Nichtnormalisierte bleiben), Datums-Modifier als Text statt als Bind in `zumAuffrischen` und `haengendeAbbrechen` | Befunde vom 23.09.2026 aus der Analyse der Nachtläufe. Die Nächte liefen sauber – sichtbar war das nur nicht: Der Verlauf zeigte fünfmal „nichts" aus dem Leerlauf nach 05:36, während Sync, Spielzeit, Besitz und 49 aufgefrischte Spiele darunter verschwanden. Dazu wuchs die Rohablage ungebremst: 2,31 von 3,26 MB und 263 KiB je Nacht, mit jedem `d1 export` erneut in die Sicherung. Keine Migration – die erste Nacht räumt den Rückstand selbst ab |
| 19 | Oberfläche: Gestaltungslinie mit Tokens (nur dunkel, Saira, eigener Zeichensatz), vier Symbole in der Leiste, Kaufliste und Lücken als Reiter der Wunschliste, Suche hinter der Lupe, Filter als Chips, Kacheln und Zeilen je Liste umschaltbar, Startseite `/start` | Alle Ansichten und Leisteneinträge existieren dann; hängt nicht an externen Freigaben. Aus einem Prototyp mit drei freien Gegenentwürfen (Regal, Kartei, Raster) entschieden – gebaut wird der abgestimmte „klassische" Entwurf, „Kartei" ist für später vorgemerkt. Das Dashboard ist herausgelöst, siehe 19a |
| 19a | Dashboard an der Stelle der Startseite: Kennzahlen je Plattform, Platin-Zähler, Backlog-Länge, letzter Sync, `trophySummary` aus PSN (7.7) | Aus Stufe 19 herausgelöst (Entscheidung des Nutzers vom 23.09.2026): Es braucht eigene Abfragen samt Lesekosten-Test und sitzt dann auf einer fertigen Gestaltung statt auf einer provisorischen. Bis dahin trägt `/start` den Hinweisblock |
| 19b | Einzeltrophäen je Spiel (7.7): Definition, eigener Stand, Zeitpunkt und weltweite Seltenheit; Erstbefüllung gestaffelt über den Cron, danach nur für Listen mit erkannter Änderung (Stufe 13); eigene Tabelle mit Index, Migration | „Was fehlt mir noch zu Platin?" im Spieldetail, Erspiel-Zeitachse, Seltenheit. Nach der Oberfläche, weil die Anzeige dann an einem fertigen Spieldetail hängt statt an einem provisorischen (Entscheidung des Nutzers vom 22.09.2026). 18 355 Trophäen in 431 Listen – unkritisch beim Speichern, solange je Spiel gelesen wird und nicht über alles |
| 20 | AWIN-Feed: Gebrauchtpreise, automatischer Physisch-Status | **Use Case 7**, Teil 1 – danach die Preisspalte in Lücken und Kaufliste |
| 21 | PSN Store-Preise | **Use Case 7**, Teil 2 |

**Reihenfolge ab Stufe 16 (Entscheidung des Nutzers vom 16.09.2026, nach Stufe 15).** Das Änderungsprotokoll rückt vor den Scanner, weil es fast jeden Schreibpfad anfasst und alles, was danach entsteht – Scanner, Cron-Sync, Feed –, von Anfang an protokolliert statt nachgerüstet wird; es bleibt eine eigene Stufe, nicht Teil des Barcode-Branches. Die Stufe „Oberfläche" (Frage des Nutzers vom 15.09.2026; bis dahin Funktion vor Form) steht nach Cron und PWA, aber **vor** den Preisstufen: 20 und 21 hängen an einer externen Freigabe beziehungsweise inoffiziellen Schnittstellen und können lange offen bleiben – so lange soll die Oberfläche nicht provisorisch bleiben. Die Preisspalte kommt danach als kleine Ergänzung.

**Stufe 16 (entschieden am 16.09.2026, Abschnitt 8.5):** Der Sync protokolliert nur, was die Änderungserkennung ohnehin feststellt – nicht jede Trophäe (Zeilenlese-Grenze); Start bei null statt Rückrechnung aus vorhandenen Stempeln; Aufbewahrung unbegrenzt, `game_event` in `EXPORT_TABELLEN`. Die Quelle je Eintrag (`nutzer` / `sync` / `igdb` / `import`, `feed` und `migration` reserviert) macht das Prinzip „Fremddaten und eigene Bewertung nie vermischen" erstmals sichtbar; geschrieben wird ausschließlich in der Repository-Schicht. **Für Stufe 17, 18 und 20:** Jeder neue Schreiber hängt sein Ereignis über `EventRepository.statement` in seinen Batch – der Scanner mit `detail = 'scan'` an `exemplar_angelegt`, der Cron-Sync über dieselben Sync-Pfade (Quelle `sync`), der Feed mit Quelle `feed`.

**Offen aus Stufe 14 (Entscheidung des Nutzers vom 16.09.2026):** IGDB belegt Disc-Fassungen nur positiv (Amazon-Artikelnummern); 249 Releases blieben `unbekannt` (208 mit Händlereinträgen ohne Medium, 13 mit Disc auf einer anderen PS-Plattform, 13 nur auf fremden Plattformen, 11 nur digital, 3 ohne Eintrag). Keine Handrecherche und keine dritte Quelle jetzt – Stufe 20 (AWIN) füllt nach; MobyGames (einzige bekannte Quelle, die auch „nur Download" sagt) bleibt als Option „14b" vermerkt, erst gegen die Restmenge zu messen.

Nach Stufe 16 sind alle Use Cases ausser 7 vollständig erfüllt – Use Case 11 seit Stufe 18 auch mit dem täglichen Statuswechsel im Cron. Stufe 20 und 21 hängen an externen Freigaben beziehungsweise inoffiziellen Schnittstellen und stehen deshalb am Ende – die Tabellen dafür existieren aber ab Stufe 1.
