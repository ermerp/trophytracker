...
**Auffrischen.** `POST /api/igdb/auffrischen` holt für die 50 am längsten nicht aktualisierten verknüpften Spiele Wertung, Cover und Datum in **einer** Anfrage (`where id = (…)`) erneut. Kritikerwertungen ändern sich mit jeder neuen Rezension; Stufe 18 hängt den Schritt an den Cron.
...
Der Schritt arbeitet wie das Auffrischen: 50 verknüpfte Spiele je Aufruf in **einer** Anfrage (`physischNachIds`, eigene Feldliste `external_games.media,external_games.platform` – die Suche schleppt die Händlerliste nicht mit), Auswahl `igdb_id IS NOT NULL` mit mindestens einem `unbekannt`-Release, das nie oder vor mehr als 30 Tagen geprüft wurde (`physical_checked_at`). Alle `unbekannt`-Releases des Spiels werden gestempelt, auch ohne Treffer und auch bei leerer Antwort – sonst fragte der nächste Aufruf dasselbe Spiel wieder an. Die Frist macht ihn wiederholbar, Stufe 18 hängt ihn an den Cron. Antwort `{ status, angefragt, gesetzt, nochOffen, weiter }`; `GET /api/igdb/status` nennt `discBelegt` und `discOffen`. Der AWIN-Feed (7.3, Stufe 20) ist die zweite Quelle; beide dürfen ein `nein` des Nutzers nicht überschreiben. Abgelehnte Spiele stehen seit Stufe 11 in `v_ohne_igdb` mit `zustand = 'abgelehnt'` und in der Ansicht „Ohne Zuordnung" hinter einem Umschalter (8.3).
...
- `angekuendigt` wird auf `erschienen` gehoben, sobald das Datum überschritten ist (`GamesRepository.erschieneneFreigeben`). Bis zum Cron in Stufe 18 geschieht das zu Beginn jedes Schritts von „Metadaten auffrischen" (`POST /api/igdb/auffrischen`, Antwort `erschienen`); der **tägliche** Lauf bleibt in Stufe 18 zu bauen. Für die Listen ist das unerheblich, weil die Views selbst vergleichen.
...
Nach Stufe 16 sind alle Use Cases ausser 7 vollständig erfüllt – Use Case 11 bis auf den täglichen Statuswechsel, der am Cron (Stufe 18) hängt. Stufe 20 und 21 hängen an externen Freigaben beziehungsweise inoffiziellen Schnittstellen und stehen deshalb am Ende – die Tabellen dafür existieren aber ab Stufe 1.
