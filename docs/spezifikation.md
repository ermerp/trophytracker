...
## 7.7 PSN jenseits der Trophäen (gemessen am 21.09.2026, Stufen 18c und 18d)
...
| `GET /api/trophy/v1/npCommunicationIds/{id}/trophyGroups/all/trophies` | Definition jeder Trophäe: Name, Beschreibung, Symbol, Art, versteckt, DLC-Gruppe | **Stufe 18d** |
| `GET /api/trophy/v1/users/me/npCommunicationIds/{id}/trophyGroups/all/trophies` | je Trophäe `earned`, Zeitpunkt, Seltenheit weltweit (`trophyEarnedRate`) | **Stufe 18d** |
| `GET /api/trophy/v1/npCommunicationIds/{id}/trophyGroups` | Hauptspiel und DLC-Gruppen mit Zählern | **Stufe 18d** |
...
| 18 | Cron Trigger `*/5 3-5 * * *` mit einem Schritt je Aufruf (10.1, Migration 0021), PWA mit Network-First-Seite und Offline-Lesezugriff auf alle Leseansichten (13) | Automatik und Komfort – der Cron-Sync ist der erste rein automatische Schreiber, den das Protokoll zeigt; Use Case 11 damit vollständig (täglicher Statuswechsel). Entscheidungen des Nutzers vom 19.09.2026: Nachtfenster, ein Versuch je Nacht, Abbruch hängender Läufe, alle Leseansichten offline, eigenes Pokal-Symbol ohne Marken. PWA abgenommen (installiert, offline geprüft), **Sync-Teil belegt** (Nachtläufe am 20. und 21.09.2026, je 431 Titel); **offen bleibt der IGDB-Schritt**, der zwei Nächte nichts tat – Stufe 18b macht ihn nachprüfbar |
...
| 18d | Einzeltrophäen je Spiel (7.7): Definition, eigener Stand, Zeitpunkt und weltweite Seltenheit; Erstbefüllung gestaffelt über den Cron, danach nur für Listen mit erkannter Änderung (Stufe 13); eigene Tabelle mit Index, Migration | „Was fehlt mir noch zu Platin?" im Spieldetail, Erspiel-Zeitachse, Seltenheit. 18 355 Trophäen in 431 Listen – unkritisch beim Speichern, solange je Spiel gelesen wird und nicht über alles |
