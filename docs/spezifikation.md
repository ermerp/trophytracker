...
| 16 | Änderungsprotokoll je Spiel: `game_event` mit Quelle je Eintrag, Block „Verlauf" im Spieldetail, Ansicht „Änderungen" | Nachvollziehbarkeit – vor allem, was danach schreibt (Idee des Nutzers vom 16.09.2026, Reihenfolge am 16.09.2026 entschieden) |
...
**Zu Stufe 16 zu klären:** Granularität beim Sync (nur, was die Änderungserkennung ohnehin feststellt – nicht jede Trophäe, Zeilenlese-Grenze), Start bei null statt Rückrechnung aus vorhandenen Stempeln, Aufbewahrung und Export (`EXPORT_TABELLEN`). Die Quelle je Eintrag (`sync` / `igdb` / `feed` / `nutzer` / `migration`) macht das Prinzip „Fremddaten und eigene Bewertung nie vermischen" erstmals sichtbar; geschrieben wird ausschließlich in der Repository-Schicht.
...
Nach Stufe 15 sind alle Use Cases ausser 7 vollständig erfüllt – Use Case 11 bis auf den täglichen Statuswechsel, der am Cron (Stufe 18) hängt. Stufe 20 und 21 hängen an externen Freigaben beziehungsweise inoffiziellen Schnittstellen und stehen deshalb am Ende – die Tabellen dafür existieren aber ab Stufe 1.
