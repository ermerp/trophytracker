...
| Titel zu Barcodes | eBay Browse API (Developer-Account), upcitemdb als Rückfall (9.2) |
...
**Zwei Betriebsarten (Entscheidung des Nutzers vom 18.09.2026):** *Zuordnen* (Vorgabe) zeigt zu jedem Code seine Karte. *Nur sammeln* schreibt erkannte Codes bloß weg – der unbekannte landet in `unresolved_scan`, der bekannte bleibt, wie er ist – und scannt sofort weiter; Rückmeldung ist ein Ton (hell = neu, zwei tiefe = schon bekannt oder in dieser Sitzung schon gescannt, tief = Fehler, `frontend/src/ton.ts`) **und ein Zeichen im Kamerabild**: Der Rahmen leuchtet für 0,9 s auf (grün neu, blau bekannt, rot Fehler), darin stehen Haken und Code, oben rechts der Zähler – eine Liste unterhalb des Bildes sieht man beim Scannen nicht (Rückmeldung des Nutzers vom 18.09.2026). Darunter die Liste der Codes dieser Sitzung, je Hülle einmal, jede Zeile mit **„verwerfen"** und darüber **„Letzten verwerfen"** – eine Fehllesung soll sofort wieder raus, nicht erst beim Zuordnen (Rückmeldung des Nutzers vom 18.09.2026). Verworfen wird nur der offene Scan; ein Code, der schon einem Release gehört, bleibt unangetastet (dafür gibt es das Lösen der Zuordnung). Das ist der erste Durchgang durch ein volles Regal, wenn die Zuordnung später in einem Rutsch passieren soll; ohne Ton – Browser ohne AudioContext, stummes Gerät – bleibt die Liste auf dem Bildschirm. Danach **pausiert** die Erkennung nach einem Treffer, bis die Karte erledigt ist (zugeordnet, „Weiter", „Später"); der zuletzt behandelte Code löst nicht erneut aus, bis ein anderer kam – sonst ersetzt die Disc, die noch vor der Kamera liegt, ihre eigene „erfasst"-Karte samt Rückgängig. Dieselbe Disc noch einmal geht über das Textfeld. Bei mehr als einer Kamera gibt es „Kamera wechseln" (Laptop mit externer Webcam); die Rückkamera des Handys ist die Vorgabe (`facingMode: environment`). Jede andere Kamera – Laptop-Webcam, Selfiekamera – wird in der **Vorschau gespiegelt** (nur die Darstellung, die Erkennung liest die echten Bilddaten): Unspiegelt wandert eine Bewegung nach rechts im Bild nach links, und man korrigiert in die falsche Richtung (Rückmeldung des Nutzers vom 18.09.2026). Solange die Kamera läuft, hält die Anwendung den **Bildschirm wach** (Wake Lock, nach dem Wegblenden neu angefordert) – im Sammelmodus wird das Gerät minutenlang nicht berührt, und mit dem Sperrbildschirm endet die Kamera; ohne die Schnittstelle (Firefox) bleibt es beim Bildschirm-Timeout. Ohne Kamera, ohne Erlaubnis oder in unsicherem Kontext bleibt das Textfeld bedienbar, mit Hinweistext.
...
- **Eine Fehllesung wirft man dort weg, wo sie auffällt.** Die Karte trägt seit dem 21.09.2026 neben „Später" ein „Verwerfen" mit Rückfrage (`DELETE /api/scan/unresolved/:ean`), denn genau in dem Moment liegt die Disc in der Hand und der falsche Code ist als falsch erkennbar. Ohne den Knopf blieb nur der Umweg über „Offene Scans", und der Code stand so lange in der Liste – die zweite Rückmeldung dieser Art (9.1).

**Offene Scans** stehen in den Einstellungen (Anzahl, zuletzt gescannt) mit „zuordnen" (öffnet `/scannen?ean=…`, Kamera bleibt aus, der Zähler bleibt – `zaehlen: false`, Rückmeldung aus der Abnahme am 17.09.2026) und „verwerfen" (`DELETE /api/scan/unresolved/:ean`, für Codes, die kein Spiel sind).
...
*Warum das in den Worker darf, upcitemdb aber nicht:* Die 10-ms-Grenze gilt für **Rechenzeit**, nicht für das Warten auf eine Antwort (Abschnitt 2). Ausschlaggebend ist allein, wie lange eine Quelle blockiert: eBay antwortet in rund 0,3 Sekunden und erlaubt 5 000 Abfragen am Tag (`GET /developer/analytics/v1_beta/rate_limit/`, geprüft am 21.09.2026), upcitemdb blockt nach je sechs Abfragen 90 Sekunden. Deshalb steht eBay in der Kette und upcitemdb im Nachtjob (9.3).
...
### 9.3 Offene Scans zuordnen (Stufe 17b)

Ein gescannter Barcode ist eine Zahl. Solange die Hülle in der Hand liegt, genügt das – im Scanner wird gleich zugeordnet. Für alles, was mit „Später" liegen blieb, fehlt der Titel: Ohne ihn lässt sich am Schreibtisch nicht sagen, welche Disc `5026555403719` war. Deshalb holt ein Job den Titel, und eine Ansicht legt ihn zur Entscheidung vor.

**Seit Stufe 17c ist der Job nur noch das Netz darunter:** Der Scanner löst unbekannte Codes live auf (9.2); der Job holt nach, was im Sammelmodus weggeschrieben wurde oder bei dem die Live-Abfrage scheiterte. Er fragt in derselben Reihenfolge wie die Kette – erst eBay, dann upcitemdb – und braucht dafür `EBAY_CLIENT_ID` und `EBAY_CLIENT_SECRET` als GitHub-Secrets; ohne sie läuft er wie vor Stufe 17c nur mit upcitemdb.

**Der Job läuft außerhalb des Workers** (`.github/workflows/scans.yml`, täglich 04:23 UTC, `scripts/scans-aufloesen.mjs`) – dieselbe Trennung wie bei den Händler-Feeds (Abschnitt 2). Grund ist die Drosselung der freien Quelle: gemessen am 18.09.2026 kam `HTTP 429` nach **jeweils sechs** Abfragen mit rund 90 Sekunden Pause, bei 100 Abfragen am Tag. Ein Job darf so lange brauchen, eine Anfrage im Worker nicht. Er holt sich die Arbeitsliste über `GET /api/scan/ungeprueft`, fragt `upcitemdb` und meldet je Code einen Titel oder `null` an `POST /api/scan/:ean/vorschlag`; beides wird mit `checked_at` vermerkt, sonst verbrauchen dieselben erfolglosen Codes jeden Tag das Kontingent. Zugeordnet wird dabei **nichts** – der Job schreibt nur Vorschläge (Abschnitt 7, kein vollautomatisches Matching).

**Gespeichert wird der Titel, nicht der Abgleich** (Migration 0020: `unresolved_scan.title_raw`, `title_source`, `checked_at`). Der Titel ist Fremddatum wie `market_offer.title_raw`. Welches Spiel der Sammlung gemeint ist, hängt dagegen an den eigenen Titeln und wäre nach einer Umbenennung oder einem neu angelegten Spiel falsch – der Abgleich entsteht deshalb beim Lesen in `GET /api/scan/unresolved`.

**Der Abgleich läuft von der Sammlung aus** (`src/domain/scan-titel.ts`). Händlertitel sind Verkäufertext: „Ps3 / Sony Playstation 3 Game – Darksiders [standard] En/ger Boxed". Ihn zu säubern hieße raten; stattdessen wird geprüft, ob **alle** Worte eines eigenen Titels darin vorkommen, nachdem Ballastworte (`ps3`, `pal`, `german`, `platinum`, `ovp` …) entfernt sind. Der längste Treffer gewinnt, wenn er die kürzeren enthält – „Killzone 3" schlägt „Killzone", „Batman: Arkham Asylum" schlägt „Batman". Bleiben zwei unabhängige Treffer, ist der Code mehrdeutig und wird vorgelegt, nicht geraten.

*Gemessen gegen die 56 PS3-Codes des Nutzers (18.09.2026):* 35 kannte `upcitemdb`, davon führten **22 zu genau einem Spiel** der Sammlung, ohne Fehlgriff; 12 lieferten einen brauchbaren Titel zu einem Spiel, das die Sammlung nicht kennt (nie gespielte Discs – dort ist der Titel die Vorlage zum Anlegen); 21 kannte die Quelle nicht. Ein Code lieferte Zahnpasta („Colgate Max Fresh Knockout"): Der Abgleich von der Sammlung aus läuft bei so etwas ins Leere, statt einen fremden Titel zu übernehmen.

**Die Ansicht `/scans`** ist wie die Import-Durchsicht (8.2) in Blöcke geteilt, nicht ein Code je Bildschirm: Wer hier arbeitet, liest jeden Vorschlag, das ist kein Blindtippen wie in der Prüfliste, und Ausgelassenes bleibt stehen (Entscheidung des Nutzers vom 18.09.2026). Die Blöcke: **Eindeutig** – genau ein Spiel, genau ein Release – mit „Erfassen" je Zeile und „Alle n erfassen"; **ohne eindeutiges Ziel** – Spiel fehlt, mehrere Treffer oder mehrere Releases – mit den Kandidaten als Knöpfe, dazu „Suchen oder anlegen" (dieselbe Komponente wie im Scanner, `ScanAuswahl`), **„Titel ist falsch"** (Vorschlag weg, Code bleibt – die Quelle lieferte am 18.09.2026 zu einem Sony-Code `711719…` Zahnpasta, der Code selbst ist richtig) und **„Scan verwerfen"** (löscht den Code endgültig, mit Rückfrage – nur für Codes, die kein Spiel sind; Frage des Nutzers vom 19.09.2026); **ohne Titel** – vom Job noch nicht gefragt, der Quelle unbekannt oder Vorschlag verworfen. Erfasst wird über `POST /api/scan/:ean/assign`, also derselbe Weg wie im Scanner: Disc mit EAN, Mapping, Protokoll „per Barcode", und offene Kauf- und Wunscheinträge am Ziel werden erledigt (Abschnitt 5). „Rückgängig" nimmt alles zurück und **stellt den offenen Scan samt Vorschlag wieder her** – sonst wäre der Code weder zugeordnet noch offen.
...
4. Sonst, mit IGDB-Zugang: `igdbAuffrischSchritt` mit Frist sieben Tage (7.6), wenn etwas fällig ist.
...
`test/keine-lecks.spec.ts` prüft sie). `GET /api/sync/status` nennt `letzterAutomatischerLauf`; die
...
POST   /api/scan                      Body: { ean, zaehlen? } – 400 bei Form oder Prüfziffer; zaehlen: false (Öffnen aus „Offene Scans") zählt nicht hoch; { ean, treffer: mapping|angebot|keiner, release? { releaseId, spielId, titel, plattform, bild, exemplare },
                                      angebot? { titel, plattform }, scans } – ohne Mapping wird der Code in unresolved_scan festgehalten (Stufe 17, 9.2)
GET    /api/scan/unresolved           { anzahl, ungeprueft, eindeutig, scans[] }, neueste zuerst; je Scan ean, scans, zuerstAm, zuletztAm,
                                      titel/quelle/geprueftAm (Vorschlag der EAN-Quelle) und der beim Lesen gebildete Abgleich:
                                      kandidaten[] { spielId, titel, releases[] } und eindeutig (9.3, Stufe 17b)
...
GET    /api/scan/ungeprueft            { eans[] } – Arbeitsliste des Jobs: Codes ohne checked_at (limit, höchstens 100)
...
POST   /api/scan/:ean/vorschlag        Body: { titel, quelle } oder { titel: null } – Titel einer EAN-Quelle festhalten; nur der Job,
                                      über das Access Service Token wie Export und Backup (15.3); 404 ohne offenen Scan
DELETE /api/scan/unresolved/:ean      offenen Scan verwerfen; 404 ohne Eintrag
...
| Offene Scans | 1 | `/scans` (seit Stufe 17b, Werkzeug in den Einstellungen, 9.3): gescannte Codes ohne Zuordnung in drei Blöcken – „Eindeutig" mit „Erfassen" je Zeile und „Alle n erfassen", „ohne eindeutiges Ziel" mit Kandidaten, Suche und „verwerfen", „ohne Titel" mit dem Stand des Jobs. Oben „Gerade erfasst" mit Rückgängig je Zeile |
