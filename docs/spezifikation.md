...
| 18c | Spielzeit und digitaler Besitz aus PSN (7.7): `gamelist/v2` für `playDuration`/`playCount`/erstes und letztes Spieldatum, Kaufliste per GraphQL mit `subscriptionService` (gekauft / PS+); Abgleich über `titelSchluessel`, nur ergänzend, kein Import; Herkunftsspalte an `digital_entitlement`; Migration | Spielzeit im Spieldetail und als Sortierkriterium (5.2), digitaler Besitz endlich ohne Handarbeit. Gemessen am 21.09.2026: 284 von 379 gespielten Titeln und 247 von 730 Käufen passen eindeutig. Entschieden am 22.09.2026: Titel ohne Spielzeit ans Listenende, `kauf` schlägt `plus`, die PS+-Seite ist eine Momentaufnahme und wird je Lauf als Ganzes ersetzt (7.7) |
