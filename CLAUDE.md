...
Keine PlayStation-Logos, -Symbole (Dreieck/Kreis/Kreuz/Quadrat, PS-Monogramm) oder -Schriftzüge in Icons, Grafiken oder Gestaltungselementen – auch nicht angedeutet oder abstrahiert (Entscheidung des Nutzers vom 19.09.2026, Abschnitt 13). Geschützte Marken, das Repository ist öffentlich. Plattformnamen als Text („PS4") sind Daten, kein Logo. Das gilt auch für **Schrift**: keine an PlayStation angelehnte Hausschrift – Sonys SST ist lizenziert, eine Nachahmung fällt unter dieselbe Regel (Wunsch des Nutzers vom 23.09.2026, abgelehnt und durch Saira ersetzt). Das App-Symbol ist ein eigener Pokal in einem Fortschrittsring (`frontend/public/icon.svg`, seit 24.09.2026).

### Gestaltung kommt aus Tokens

Farben, Abstände, Radien und Schriften stehen in `frontend/src/tokens.css` und werden nur als `var(--…)` benutzt. Kein Literalwert in `App.css`, keine Systemfarbe wie `Canvas`. Vor Stufe 19 lagen Farben als `rgba(128, 128, 128, 0.3)` verstreut in 1 286 Zeilen – dieselbe Linie war an keiner Stelle nachvollziehbar. Die Anwendung ist **nur dunkel**; ein zweiter Tokensatz ist eine Entscheidung des Nutzers, keine Ergänzung nebenbei.
...
- Schwere Importe (Händler-Feeds) laufen in einer GitHub Action, nicht im Worker. Maßstab ist die **Blockierdauer der Quelle, nicht das Wort „extern"**: Netzwartezeit zählt nicht gegen die 10 ms. Deshalb läuft die eBay Browse API seit Stufe 17c live im Worker (0,3 s, 5 000/Tag, `src/ebay/client.ts`) und seit 17d auch upcitemdb als Rückfall (`src/ean/upcitemdb.ts`) – es drosselt zwar hart (90 s nach je sechs Abfragen), wird aber nur selten gefragt und wirft nie: ein Fehler heißt „kein Titel"
...
- **Kein Bind in `datetime('now', ?)`.** Ein Datums-Modifier gehört als Text ins Statement (`datetime('now', '-7 days')`); die Zahl kommt aus einer Konstanten, nie aus einer Eingabe. **Die Begründung war bis zum 23.09.2026 zu stark:** Als der nächtliche IGDB-Schritt zwei Nächte schwieg (Stufe 18b), war der Bind einer von drei Verdächtigen, und alle drei wurden zugleich geändert – hier stand er danach als gemessene Ursache. Er war es nicht. Die auswählende Abfrage (`zumAuffrischen`) band weiterhin, weil die Umstellung von 18b nur die Zählabfrage der Statusanzeige getroffen hatte, und frischte trotzdem jede Nacht auf (49 Spiele am 23.09.2026). Belegt sind die beiden anderen Maßnahmen: der fehlende Stempel für Spiele, die IGDB nicht zurückgibt, und das fehlende `try/catch` um die IGDB-Schritte. Seit Stufe 18d steht der Modifier überall als Text – die Regel gilt als Stil, sie erklärt aber keinen Ausfall. **Die Lehre daraus ist die größere:** Werden mehrere Maßnahmen gegen einen Fehler zugleich ausgeliefert, ist hinterher keine davon belegt. Was als Vermutung gebaut wurde, wird auch als Vermutung aufgeschrieben
...
- **Oberfläche wird angesehen, nicht nur gebaut.** Vor dem Deploy die geänderten Ansichten rendern und das Bild prüfen: headless Chrome (`~/.cache/ms-playwright/chromium-1234/chrome-linux64/chrome --headless=new --window-size=390,844 --screenshot=…`) gegen `wrangler dev`, die lokale D1 mit **erfundenen** Zeilen gefüllt. In Stufe 19 fand das drei Fehler, die beim Lesen des Codes unsichtbar waren: ausgeblendete Navigationspunkte, die doch erschienen, eine Knopfreihe, die dem Titel 66 Pixel liess, und fünf Symbolknöpfe in einer 171 px breiten Kachel. Und ein **gemeldeter Darstellungsfehler wird erst belegt, dann behoben** – `viewport-fit=cover` ging auf Verdacht live und war es nicht; erst Bildschirmfoto und Pixelmessung zeigten, dass der Strich ausserhalb der Seite liegt.
